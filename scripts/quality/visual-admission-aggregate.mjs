...
function validateExecutableIdentityPath(path) {
  if (
    ...
    !supportedExecutableIdentityExtensions.has(extname(path))
  )
...
function validateExecutableIdentityRegistry(registry) {
  if (
    ...
    registry.schema !== 'fe058-admission/executable-identity-registry-v1' ||
    !Array.isArray(registry.paths)
  )
    throw new Error('Malformed executable identity registry.');
  if (registry.paths.length !== 33)
    throw new Error(
      `Executable identity registry must contain exactly 33 paths, found ${registry.paths.length}.`,
    );
  const seen = new Set();
  ...
  if (!seen.has(executableIdentityRegistryPath))
    throw new Error('Executable identity registry must bind its own path.');
  if (!seen.has(publicCommandManifestPath))
    throw new Error('Executable identity registry must bind the public command manifest.');
...
const targetFiles = validateExecutableIdentityRegistry(
  JSON.parse(readFileSync(join(root, executableIdentityRegistryPath), 'utf8')),
);
...
if (!['canonical', 'full'].includes(screenshotMode))
  throw new Error(`Unsupported screenshot mode: ${screenshotMode}`);
...
function writeTerminalSuccess(report) {
  ...
  writeFileSync(
    terminalPath,
    `${JSON.stringify({ schema: 'fe058-admission/aggregate-terminal-v1', runId, status: 'complete', report }, null, 2)}\n`,
    { flag: 'wx' },
  );
}
...
function publishAcceptedOutputs({
  ...
  buildManifest,
  generationId,
  resultSnapshotPathPrefix = resultsRelativeRoot,
  failurePhase = '',
  assertInputsCurrent = () => {},
  onPublicationPhase = () => {},
}) {
  ...
  const manifestRecord = buildManifest({
  ...
  if (manifestRecord.result_snapshot) {
    ...
    if (!terminalSnapshot)
      throw new Error('Missing pre-publication canonical result snapshot terminal contract.');
    const verifiedSnapshot = verifyCanonicalResultSnapshotDigestContract(
      terminalSnapshot,
      resultSnapshotPathPrefix,
    );
    if (
      ...
      manifestRecord.result_snapshot.digest_algorithm_version !==
        terminalSnapshot.digestAlgorithm.version ||
      !orderedStringArraysEqual(
        manifestRecord.result_snapshot.digest_role_enumeration,
        terminalSnapshot.digestAlgorithm.roleEnumeration,
      ) ||
  ...
  const previous = new Map(
    targets.map((path) => [path, existsSync(path) ? readFileSync(path) : undefined]),
  );
  ...
  try {
    ...
    staged.set(
      acceptedTerminalPath,
      stageDurableFile(acceptedTerminalPath, terminalBytes, generationId),
    );
    ...
    if (failurePhase === 'after-stage')
      throw new Error('Controlled atomic publication failure after staging.');
    ...
    if (failurePhase === 'after-terminal')
      throw new Error('Controlled atomic publication failure after terminal replacement.');
    ...
    if (failurePhase === 'after-report')
      throw new Error('Controlled atomic publication failure after report replacement.');
    ...
    assertInputsCurrent();
    if (failurePhase === 'after-manifest')
      throw new Error('Controlled atomic publication failure after manifest replacement.');
    verifyCanonicalOutputBinding(
      manifestPath,
      reportPath,
      acceptedTerminalPath,
      generationId,
    ...
    if (rollbackError)
      throw new Error(
        `Atomic publication failed and rollback was incomplete: ${String(error)}; ${String(rollbackError)}`,
      );
    throw error;
  } finally {
...
function verifyCanonicalOutputBinding(
  manifestPath,
  reportPath,
  acceptedTerminalPath,
  generationId,
  ...
  if (
    ...
    manifest.terminal.sha256 !== sha256(terminalBytes) ||
    manifest.close_or_route.sha256 !== sha256(readFileSync(reportPath))
  )
  ...
  if (manifest.result_snapshot) {
    ...
    if (
      ...
      manifest.result_snapshot.digest_algorithm_version !==
        terminalSnapshot.digestAlgorithm.version ||
      !orderedStringArraysEqual(
        manifest.result_snapshot.digest_role_enumeration,
        terminalSnapshot.digestAlgorithm.roleEnumeration,
      ) ||
...
if (atomicOutputProof) {
  ...
  if (!resolve(proofRoot).startsWith(`${resolvedTempRoot}${sep}`))
    throw new Error(`Refusing atomic proof outside the system temporary directory: ${proofRoot}`);
  try {
    ...
    try {
      publishAcceptedOutputs({
        ...
        terminalRecord: {
          schema: 'fe058-admission/aggregate-terminal-v1',
          runId: 'proof-run',
          status: 'complete',
          report: { generationId },
        },
      ...
      if (!failurePhase || !String(error).includes('Controlled atomic publication failure'))
        throw error;
      rejected = true;
    }
    if (failurePhase) {
      if (!rejected)
        throw new Error(`Atomic output negative proof unexpectedly passed: ${failurePhase}`);
      for (const [path, bytes] of oldBytes)
        if (!readFileSync(path).equals(bytes))
          throw new Error(
            `Atomic output negative proof replaced ${basename(path)} during ${failurePhase}.`,
          );
    } else {
      verifyCanonicalOutputBinding(
        proofManifestPath,
    ...
    process.stdout.write(
      `${JSON.stringify({ atomicOutputProof, accepted: !failurePhase, rejected, canonicalOutputsUnchanged: Boolean(failurePhase) })}\n`,
    );
...
if (snapshotPublicationProof) {
  ...
  if (!resolve(proofRoot).startsWith(`${resolvedTempRoot}${sep}`))
    throw new Error(
      `Refusing snapshot publication proof outside the system temporary directory: ${proofRoot}`,
    );
  try {
    ...
    const assertInputsCurrent = () => {
      if (
        !existsSync(proofInputPath) ||
        sha256(readFileSync(proofInputPath)) !== expectedInputSha256
      )
    ...
    if (driftPhase) {
      if (!rejected)
        throw new Error(`Snapshot publication drift proof unexpectedly passed: ${driftPhase}`);
      for (const [path, bytes] of oldBytes)
        if (!readFileSync(path).equals(bytes))
          throw new Error(
            `Snapshot publication drift proof replaced ${basename(path)} during ${driftPhase}.`,
          );
    } else {
      verifyCanonicalOutputBinding(
        proofManifestPath,
    ...
    process.stdout.write(
      `${JSON.stringify({ snapshotPublicationProof, accepted: !driftPhase, rejected, canonicalOutputsUnchanged: Boolean(driftPhase) })}\n`,
    );
    rmSync(proofRoot, { recursive: true, force: true });
    process.exit(0);
  } finally {
...
const expectedCellIds = new Set(
  contexts.flatMap(([matrix, scenario, scenarioLocales]) =>
    scenarioLocales.flatMap((locale) =>
      [
...
function expectedCapturedCellIdsForMode(mode) {
  if (!['canonical', 'full'].includes(mode))
    throw new Error(`Unsupported screenshot mode: ${mode}`);
  return new Set(
    [...expectedCellIds].filter((cellId) => {
...
function expectedShardDirectoryNames(expectedRunId) {
  return [
    ...
    `m04-${expectedRunId}`,
    `m06-${expectedRunId}`,
    ...cartShardKeys.map((key) => `m07-${key}-${expectedRunId}`),
  ];
}
...
const resultSnapshotRoleEnumeration = Object.freeze([
  'terminal-marker',
  'cell-result',
  'screenshot',
  'm01-outcome',
  'declared-pending-release',
]);
const resultSnapshotDigestAlgorithm = Object.freeze({
  ...
  pathSyntax:
    'relative slash-separated path; no empty, dot, dot-dot, drive-prefix, backslash, or colon segment syntax',
...
const expectedResultSnapshotRoleCounts =
  expectedResultSnapshotRoleCountsForCapturedCells(expectedCapturedCellIds);
...
function validateResultSnapshotRoleExtension(path, role) {
  if (!resultSnapshotRoles.has(role))
    throw new Error(`Unsupported result snapshot role: ${role || '(empty)'}`);
  const extension = extname(path);
  if (
    (role === 'screenshot' && extension !== '.png') ||
    (role !== 'screenshot' && extension !== '.json')
  )
...
function validateCanonicalResultSnapshotDigestHeader(schema, digestAlgorithm) {
  ...
  for (const role of digestAlgorithm.roleEnumeration) {
    ...
    if (uniqueRoles.has(role))
      throw new Error(`Duplicate canonical result snapshot role enumeration value: ${role}.`);
    uniqueRoles.add(role);
  }
}

function validateCanonicalResultSnapshotEntries(schema, count, entries, pathPrefix = '') {
  if (schema !== resultSnapshotSchema)
    throw new Error(`Unsupported canonical result snapshot schema: ${schema || '(empty)'}.`);
  if (
  ...
  for (const entry of entries) {
    if (
      ...
      !['path', 'size', 'sha256', 'role'].every((key) => Object.hasOwn(entry, key))
    )
    ...
    if (!Number.isSafeInteger(entry.size) || entry.size < 0)
      throw new Error(`Malformed result snapshot size: ${entry.path}.`);
    if (typeof entry.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(entry.sha256))
      throw new Error(`Malformed result snapshot SHA-256: ${entry.path}.`);
    if (priorPath !== undefined && compareCanonicalPaths(priorPath, entry.path) >= 0)
      throw new Error(
        `Result snapshot entries are not in unique ascending UTF-8 byte order: ${entry.path}.`,
      );
    priorPath = entry.path;
  }
}

function encodeCanonicalResultSnapshotPreimage(
  schema,
  count,
  entries,
  pathPrefix = '',
  ...
  for (const entry of entries)
    lines.push(`entry\t${entry.path}\t${entry.size}\t${entry.sha256}\t${entry.role}`);
  return Buffer.from(`${lines.join('\n')}\n`, 'utf8');
}

function recomputeCanonicalResultSnapshotDigestIndependently(
  schema,
  count,
  entries,
  pathPrefix = '',
...
function computeCanonicalResultSnapshotDigest(
  schema,
  count,
  entries,
  pathPrefix = '',
  ...
  const preimage = encodeCanonicalResultSnapshotPreimage(
    schema,
    count,
    entries,
    pathPrefix,
  ...
  const independent = recomputeCanonicalResultSnapshotDigestIndependently(
    schema,
    count,
    entries,
    pathPrefix,
  ...
  if (
    production.digest !== independent.digest ||
    production.preimageByteLength !== independent.preimageByteLength
  )
  ...
  for (const [key, value] of Object.entries(resultSnapshotDigestAlgorithm)) {
    if (Array.isArray(value)) {
      if (
        ...
        candidate[key].length !== value.length ||
        value.some((item, index) => candidate[key][index] !== item)
      )
...
function verifyCanonicalResultSnapshotDigestContract(contract, pathPrefix = '') {
  if (!contract || typeof contract !== 'object' || Array.isArray(contract))
    throw new Error('Missing canonical result snapshot contract.');
  assertCanonicalResultSnapshotDigestAlgorithm(contract.digestAlgorithm);
  const computed = computeCanonicalResultSnapshotDigest(
    contract.schema,
    contract.count,
    contract.entries,
    pathPrefix,
  ...
  if (
    !Number.isSafeInteger(contract.preimageByteLength) ||
    contract.preimageByteLength !== computed.preimageByteLength ||
    contract.digest !== computed.digest
  )
...
function buildResultSnapshotContract(candidates, pathPrefix) {
  ...
  for (const candidate of candidates) {
    ...
    if (!Buffer.isBuffer(candidate.bytes))
      throw new Error(`Missing result snapshot bytes: ${candidate.path}`);
    if (exactPaths.has(candidate.path))
      throw new Error(`Duplicate result snapshot path: ${candidate.path}`);
    const caseFoldedPath = candidate.path.toLocaleLowerCase('en-US');
    if (caseFoldedPaths.has(caseFoldedPath))
      throw new Error(`Case-colliding result snapshot path: ${candidate.path}`);
  ...
  const digest = computeCanonicalResultSnapshotDigest(
...
function classifyResultSnapshotRole(shardDirectory, filePath) {
  ...
  if (
    !shardRelativePath ||
    shardRelativePath === '..' ||
    shardRelativePath.startsWith('../') ||
    isAbsolute(shardRelativePath)
  )
  ...
  if (
    ...
    segments[2].endsWith('.png')
  )
    return 'screenshot';
  if (segments.length === 2 && segments[0] && segments[1] === 'm01-outcome.json')
    return 'm01-outcome';
  if (segments.length === 2 && segments[0] && segments[1] === 'declared-pending-release.json')
    return 'declared-pending-release';
...
function validateCanonicalResultSnapshotInventory(
  contract,
  terminalShards,
  pathPrefix,
  expectedRoleCounts = expectedResultSnapshotRoleCounts,
) {
  ...
  for (const [role, expectedCount] of expectedRoleCounts) {
    if (counts.get(role) !== expectedCount)
      throw new Error(
        `Canonical result snapshot ${role} count mismatch: expected=${expectedCount} observed=${counts.get(role)}.`,
      );
  }
  const markerPaths = new Set(
    contract.entries.filter(({ role }) => role === 'terminal-marker').map(({ path }) => path),
  );
  for (const shard of terminalShards) {
    const expectedMarkerPath = `${pathPrefix}/${basename(shard.resultDirectory)}/.last-run.json`;
    if (!markerPaths.has(expectedMarkerPath))
      throw new Error(`Missing canonical terminal marker snapshot: ${expectedMarkerPath}`);
  }
  if (
    contract.count !== [...expectedRoleCounts.values()].reduce((total, count) => total + count, 0)
  )
    throw new Error(`Canonical result snapshot file count mismatch: ${contract.count}.`);
}

function captureCanonicalResultSnapshot(
  candidateDirectories,
  expectedRunId,
  {
    resultsRoot: snapshotResultsRoot = resultsRoot,
    pathPrefix = resultsRelativeRoot,
    expectedCapturedCells = expectedCapturedCellIds,
  } = {},
) {
  const terminalShards = validateTerminalDirectories(
    candidateDirectories,
    expectedRunId,
    snapshotResultsRoot,
  );
  ...
  validateCanonicalResultSnapshotInventory(
    contract,
    terminalShards,
    pathPrefix,
    expectedResultSnapshotRoleCountsForCapturedCells(expectedCapturedCells),
  );
  ...
  const capturedTerminalShards = terminalShards.map((shard) => {
    ...
    if (!marker || marker.status !== 'passed')
      throw new Error(`Non-passed terminal marker: ${markerPath}`);
  ...
  return {
    ...
    buffers,
    entriesByPath,
    terminalShards: capturedTerminalShards,
    options: { resultsRoot: snapshotResultsRoot, pathPrefix },
  };
}

function assertCanonicalResultSnapshotCurrent(snapshot, candidateDirectories, expectedRunId) {
  const refreshed = captureCanonicalResultSnapshot(
    candidateDirectories,
    expectedRunId,
    snapshot.options,
  );
...
function parseCanonicalResultJson(snapshot, path, expectedRole) {
  ...
  if (!entry || entry.role !== expectedRole)
    throw new Error(`Missing canonical ${expectedRole} snapshot: ${logicalPath}`);
  try {
...
function resolveCanonicalScreenshot(snapshot, referenceBaseDirectory, screenshotReference) {
  if (
    typeof screenshotReference !== 'string' ||
    !screenshotReference ||
    isAbsolute(screenshotReference)
  )
    throw new Error(
      `Malformed canonical screenshot reference: ${screenshotReference || '(empty)'}`,
    );
  ...
  const entry = snapshot.entriesByPath.get(logicalPath);
  if (!entry || entry.role !== 'screenshot')
    throw new Error(`Missing canonical screenshot snapshot: ${logicalPath}`);
  return entry;
}
...
function validateTerminalDirectories(
  candidateDirectories,
  expectedRunId,
  snapshotResultsRoot = resultsRoot,
) {
  ...
  if (candidateDirectories.length !== expectedNames.length)
    throw new Error(
      `Expected exactly ${expectedNames.length} terminal result directories, found ${candidateDirectories.length}.`,
    );
  ...
  for (let index = 0; index < expectedNames.length; index += 1) {
    ...
    if (actualDirectory !== expectedDirectory)
      throw new Error(
        `Unexpected terminal result directory at index ${index}: ${candidateDirectories[index]}`,
      );
    if (lstatSync(actualDirectory).isSymbolicLink())
      throw new Error(`Terminal result directory aliases are forbidden: ${actualDirectory}`);
    ...
    try {
      ...
      throw new Error(`Malformed terminal marker: ${markerPath}`);
    }
    if (!marker || marker.status !== 'passed')
      throw new Error(`Non-passed terminal marker: ${markerPath}`);
    terminalShards.push({
...
function assertTerminalShardSnapshotCurrent(terminalShards) {
  for (const shard of terminalShards) {
    if (
      !existsSync(shard.terminalMarker) ||
      sha256(readFileSync(shard.terminalMarker)) !== shard.terminalSha256
    )
...
if (aggregateProbe === 'missing-terminal') {
  process.stdout.write(
    `${JSON.stringify({ schema: 'fe058-admission/aggregate-v2-probe', runId, result: 'complete-without-terminal' })}\n`,
  );
  process.exit(0);
}
if (aggregateProbe === 'stale-terminal') {
  writeFileSync(
    terminalPath,
    `${JSON.stringify({ schema: 'fe058-admission/aggregate-terminal-v1', runId: `${runId}-stale`, status: 'complete' })}\n`,
    { flag: 'wx' },
  );
  process.stdout.write(
    `${JSON.stringify({ schema: 'fe058-admission/aggregate-v2-probe', runId, result: 'complete-with-stale-terminal' })}\n`,
  );
  process.exit(0);
}
export function validateInteractionRecord(record) {
  ...
  if (
    ...
    !interaction.target ||
    !['focus_transition', 'modal_focus_containment'].includes(interaction.mode) ||
    !['focus', 'keyboard', 'minTarget44'].every(
      (name) =>
        interaction[name] && ['pass', 'fail', 'not_applicable'].includes(interaction[name].status),
    )
  )
    throw new Error(`Malformed interaction record: ${record.cellId}`);
  const isPendingContainment =
    record.matrix === 'M08' &&
    record.scenario === 'clear-pending' &&
    record.state === 'clear-pending' &&
    record.session === 'authenticated';
  if (interaction.mode === 'modal_focus_containment') {
    if (!isPendingContainment)
      throw new Error(`Modal containment outside exact pending M08 state: ${record.cellId}`);
    const modal = interaction.modal;
    if (
      !modal ||
      typeof modal.initialDialogFocus !== 'boolean' ||
      typeof modal.tabFocusWithinDialog !== 'boolean' ||
      typeof modal.noBackgroundFocus !== 'boolean' ||
      !Number.isInteger(modal.enabledDialogActions)
    )
      throw new Error(`Malformed modal containment evidence: ${record.cellId}`);
    if (modal.enabledDialogActions !== 0)
      throw new Error(`Modal containment claimed with enabled dialog action: ${record.cellId}`);
    if (!modal.initialDialogFocus || !modal.tabFocusWithinDialog || !modal.noBackgroundFocus)
      throw new Error(`Failed modal focus containment: ${record.cellId}`);
  ...
  for (const name of ['focus', 'keyboard', 'minTarget44']) {
    ...
    if (record.matrix === 'M09') {
      ...
      throw new Error(
        `Required applicable interaction failed: ${record.cellId} ${name}=${check.status}`,
      );
    }
  }
}
export function validateNavigationTeardowns(record) {
  ...
  if (!Array.isArray(teardowns))
    throw new Error(`Malformed navigation teardown diagnostics: ${record.cellId ?? 'M01 outcome'}`);
  const seen = new Set();
  for (const teardown of teardowns) {
    if (
      record.matrix !== 'M01' ||
      record.scenario !== 'completion-ready' ||
      record.route !== '/learning/enrollments/4'
    )
      throw new Error(
        `Unexpected locale-reload teardown context: ${record.cellId ?? 'M01 outcome'}`,
      );
    const boundary = teardown?.boundary;
    if (
      !Number.isInteger(teardown?.requestId) ||
      teardown.requestId < 1 ||
      teardown?.method !== 'GET' ||
      teardown?.error !== 'net::ERR_ABORTED' ||
      !boundary ||
      !['capture-route-navigation', 'locale-reload'].includes(boundary.cause) ||
      !['en', 'ru', 'uz'].includes(boundary.sourceLocale) ||
      !['en', 'ru', 'uz'].includes(boundary.targetLocale) ||
      !Number.isInteger(boundary.epoch) ||
      boundary.epoch < 1 ||
      boundary.phase !== 'pre_document_commit' ||
      boundary.matrix !== record.matrix ||
      boundary.scenario !== record.scenario ||
      boundary.route !== record.route ||
      boundary.targetLocale !== record.locale ||
      teardown?.preNavigationRequest !== true
    )
      throw new Error(`Malformed navigation teardown proof: ${record.cellId ?? 'M01 outcome'}`);
    if (
      boundary.cause === 'capture-route-navigation' &&
      boundary.sourceLocale !== boundary.targetLocale
    )
      throw new Error(
        `Route-navigation locale provenance does not preserve the displayed locale: ${record.cellId ?? 'M01 outcome'}`,
      );
    const bootstrap = boundary.bootstrap;
    if (
      bootstrap !== undefined &&
      (boundary.cause !== 'locale-reload' ||
        boundary.epoch !== 1 ||
        boundary.sourceLocale !== 'en' ||
        boundary.targetLocale !== record.locale ||
        bootstrap.kind !== 'initial-default-en-missing-storage' ||
        bootstrap.observedDocumentLocale !== 'en' ||
        bootstrap.observedStorage !== 'missing')
    )
      throw new Error(
        `Malformed initial locale bootstrap provenance: ${record.cellId ?? 'M01 outcome'}`,
      );
    ...
    if (!allowedLocaleReloadTeardowns.has(allowlistIdentity))
      throw new Error(
        `Unapproved locale-reload teardown: ${record.cellId ?? 'M01 outcome'} ${allowlistIdentity}`,
      );
    if (seen.has(identity))
      throw new Error(
        `Duplicate locale-reload teardown: ${record.cellId ?? 'M01 outcome'} ${identity}`,
      );
    seen.add(identity);
  }
}
export function validateRequestLifecycles(record) {
  ...
  if (!Array.isArray(lifecycles))
    throw new Error(`Missing request lifecycle diagnostics: ${record.cellId}`);
  const requestIds = new Set();
  for (const lifecycle of lifecycles) {
    const context = lifecycle?.context;
    if (
      !Number.isInteger(lifecycle?.requestId) ||
      lifecycle.requestId < 1 ||
      requestIds.has(lifecycle.requestId) ||
      !Number.isInteger(lifecycle.sequence) ||
      lifecycle.sequence < 1 ||
      typeof lifecycle.method !== 'string' ||
      !lifecycle.method ||
      typeof lifecycle.path !== 'string' ||
      !lifecycle.path.startsWith('/') ||
      !Number.isInteger(lifecycle.captureWindowId) ||
      lifecycle.captureWindowId < 1 ||
      !Number.isInteger(lifecycle.navigationEpoch) ||
      lifecycle.navigationEpoch < 1 ||
      !context ||
      context.matrix !== record.matrix ||
      context.scenario !== record.scenario ||
      context.route !== record.route ||
      context.state !== record.state ||
      context.session !== record.session ||
      context.disposition !== record.disposition ||
      !['en', 'ru', 'uz'].includes(lifecycle.sourceLocale) ||
      lifecycle.targetLocale !== record.locale ||
      !['initial-navigation', 'locale-reload', 'capture-route-navigation'].includes(
        lifecycle.cause,
      ) ||
      !['pre_document_commit', 'post_document_commit'].includes(lifecycle.phase) ||
      !['response', 'failure'].includes(lifecycle.outcome)
    )
      throw new Error(`Malformed or cross-window request lifecycle: ${record.cellId}`);
    if (
      lifecycle.outcome === 'response'
        ? !Number.isInteger(lifecycle.status)
        : typeof lifecycle.error !== 'string' || !lifecycle.error
    )
...
export function validateSupersededReadLifecycles(record) {
  ...
  if (!Array.isArray(pairs))
    throw new Error(`Missing superseded-read diagnostics: ${record.cellId}`);
  const failedIds = new Set();
  const replacementIds = new Set();
  for (const pair of pairs) {
    const failed = record.diagnostics.requestLifecycles.find(
      (lifecycle) => lifecycle.requestId === pair?.failedRequestId,
    );
    const replacement = record.diagnostics.requestLifecycles.find(
      (lifecycle) => lifecycle.requestId === pair?.replacementRequestId,
    );
    if (
      !failed ||
      !replacement ||
      pair.failedRequestId === pair.replacementRequestId ||
      failedIds.has(pair.failedRequestId) ||
      replacementIds.has(pair.replacementRequestId) ||
      pair.method !== 'GET' ||
      pair.path !== failed.path ||
      pair.failedSequence !== failed.sequence ||
      pair.replacementSequence !== replacement.sequence ||
      replacement.sequence <= failed.sequence ||
      pair.captureWindowId !== failed.captureWindowId ||
      pair.captureWindowId !== replacement.captureWindowId ||
      pair.navigationEpoch !== failed.navigationEpoch ||
      pair.navigationEpoch !== replacement.navigationEpoch ||
      JSON.stringify(pair.context) !== JSON.stringify(failed.context) ||
      JSON.stringify(pair.context) !== JSON.stringify(replacement.context) ||
      pair.sourceLocale !== failed.sourceLocale ||
      pair.sourceLocale !== replacement.sourceLocale ||
      pair.targetLocale !== failed.targetLocale ||
      pair.targetLocale !== replacement.targetLocale ||
      pair.cause !== failed.cause ||
      pair.cause !== replacement.cause ||
      pair.phase !== failed.phase ||
      pair.phase !== replacement.phase ||
      failed.method !== 'GET' ||
      failed.outcome !== 'failure' ||
      failed.error !== 'net::ERR_ABORTED' ||
      replacement.method !== 'GET' ||
      replacement.path !== failed.path ||
      replacement.outcome !== 'response' ||
      replacement.status !== pair.replacementStatus ||
      replacement.status < 200 ||
      replacement.status >= 300 ||
      pair.targetRendered !== true
    )
      throw new Error(`Malformed superseded-read lifecycle: ${record.cellId}`);
    failedIds.add(pair.failedRequestId);
    replacementIds.add(pair.replacementRequestId);
  }
  const separatelyProvenFailures = new Set([
    ...(record.diagnostics.navigationTeardowns ?? []).map((teardown) => teardown.requestId),
    ...(record.diagnostics.catalogHeroLifecycles ?? [])
      .filter((event) => event.kind === 'aborted')
      .map((event) => event.requestId),
  ]);
  for (const lifecycle of record.diagnostics.requestLifecycles)
    if (
      lifecycle.outcome === 'failure' &&
      lifecycle.error === 'net::ERR_ABORTED' &&
      !failedIds.has(lifecycle.requestId) &&
      !separatelyProvenFailures.has(lifecycle.requestId)
    )
      throw new Error(
        `Real current-window request failure: ${record.cellId} ${lifecycle.method} ${lifecycle.path} ${lifecycle.error}`,
      );
}
export function validateRetiredSetupLifecycleEvidence(record) {
  const evidence = record.diagnostics?.retiredSetupLifecycleEvidence;
  const recordContext = {
  ...
  const requiresEvidence = record.matrix === 'M04' || record.matrix === 'M07';
  if (!requiresEvidence) {
    if (evidence !== undefined)
      throw new Error(`Unexpected retired setup lifecycle evidence: ${record.cellId}`);
    return;
  }
  if (evidence === null) {
    if (
      requiresEvidence &&
      record.diagnostics.requestLifecycles.every((lifecycle) => lifecycle.cause === 'locale-reload')
    )
      return;
    throw new Error(`Malformed ordinary locale reload evidence: ${record.cellId}`);
  }
  if (!evidence) throw new Error(`Missing retired setup lifecycle evidence: ${record.cellId}`);
  if (
    !Array.isArray(evidence.setupLifecycles) ||
    !Array.isArray(evidence.replacementLifecycles) ||
    !Array.isArray(evidence.replacementPairs)
  )
    throw new Error(`Missing retired setup lifecycle evidence: ${record.cellId}`);
  const setup = evidence.setupWindow;
  const reload = evidence.reloadWindow;
  if (
    !setup ||
    !reload ||
    setup.captureWindowId === reload.captureWindowId ||
    setup.navigationEpoch === reload.navigationEpoch ||
    setup.cause !== 'initial-navigation' ||
    reload.cause !== 'locale-reload' ||
    JSON.stringify(setup.context) !== JSON.stringify(recordContext) ||
    JSON.stringify(reload.context) !== JSON.stringify(recordContext) ||
    reload.targetLocale !== record.locale ||
    setup.targetLocale !== reload.sourceLocale
  )
    throw new Error(`Malformed retired setup window relationship: ${record.cellId}`);
  validateRequestLifecycles({
    ...record,
    diagnostics: { requestLifecycles: evidence.replacementLifecycles },
  });
  const setupIds = new Set();
  const aborted = [];
  for (const lifecycle of evidence.setupLifecycles) {
    if (
      !Number.isInteger(lifecycle?.requestId) ||
      setupIds.has(lifecycle.requestId) ||
      lifecycle.captureWindowId !== setup.captureWindowId ||
      lifecycle.navigationEpoch !== setup.navigationEpoch ||
      JSON.stringify(lifecycle.context) !== JSON.stringify(setup.context) ||
      lifecycle.sourceLocale !== setup.sourceLocale ||
      lifecycle.targetLocale !== setup.targetLocale ||
      lifecycle.cause !== setup.cause ||
      lifecycle.phase !== setup.phase ||
      lifecycle.method !== 'GET' ||
      (lifecycle.outcome === 'response'
        ? !Number.isInteger(lifecycle.status) || lifecycle.status < 200 || lifecycle.status >= 300
        : lifecycle.outcome !== 'failure' || lifecycle.error !== 'net::ERR_ABORTED')
    )
      throw new Error(`Malformed retired setup terminal lifecycle: ${record.cellId}`);
    setupIds.add(lifecycle.requestId);
    if (lifecycle.outcome === 'failure') aborted.push(lifecycle);
  }
  const replacementIds = new Set();
  if (evidence.replacementPairs.length !== aborted.length)
    throw new Error(`Unmatched retired setup abort: ${record.cellId}`);
  for (const pair of evidence.replacementPairs) {
    const old = evidence.setupLifecycles.find(
      (lifecycle) => lifecycle.requestId === pair?.setupRequestId,
    );
    const replacement = evidence.replacementLifecycles.find(
      (lifecycle) => lifecycle.requestId === pair?.replacementRequestId,
    );
    if (
      !old ||
      !replacement ||
      replacementIds.has(pair.replacementRequestId) ||
      pair.setupRequestId === pair.replacementRequestId ||
      pair.setupSequence !== old.sequence ||
      pair.replacementSequence !== replacement.sequence ||
      ...
      old.outcome !== 'failure' ||
      old.error !== 'net::ERR_ABORTED' ||
      replacement.outcome !== 'response' ||
      replacement.status !== pair.replacementStatus ||
      replacement.status < 200 ||
      replacement.status >= 300 ||
      replacement.captureWindowId !== reload.captureWindowId ||
      replacement.navigationEpoch !== reload.navigationEpoch ||
      JSON.stringify(replacement.context) !== JSON.stringify(reload.context) ||
      replacement.sourceLocale !== reload.sourceLocale ||
      replacement.targetLocale !== reload.targetLocale ||
      replacement.cause !== reload.cause ||
      replacement.phase !== reload.phase
    )
      throw new Error(`Malformed retired setup replacement pair: ${record.cellId}`);
    replacementIds.add(pair.replacementRequestId);
  }
}
export function validateDeclaredPendingRequests(record) {
  ...
  if (!Array.isArray(pending))
    throw new Error(`Missing declared pending diagnostics: ${record.cellId}`);
  const isPendingM08 =
    record.matrix === 'M08' &&
    record.scenario === 'clear-pending' &&
    record.state === 'clear-pending' &&
  ...
  if (!isPendingM08) {
    if (pending.length)
      throw new Error(
        `Declared pending request outside the exact M08 pending context: ${record.cellId}`,
      );
    return undefined;
  }
  if (pending.length !== 1)
    throw new Error(`Expected exactly one declared pending request: ${record.cellId}`);
  const snapshot = pending[0];
  const context = snapshot?.context;
  if (
    !Number.isInteger(snapshot?.requestId) ||
    snapshot.requestId < 1 ||
    snapshot.method !== 'DELETE' ||
    snapshot.path !== '/cart' ||
    !Number.isInteger(snapshot.captureWindowId) ||
    snapshot.captureWindowId < 1 ||
    !Number.isInteger(snapshot.navigationEpoch) ||
    snapshot.navigationEpoch < 1 ||
    !context ||
    context.matrix !== record.matrix ||
    context.scenario !== record.scenario ||
    context.route !== record.route ||
    ...
    context.session !== record.session ||
    context.disposition !== record.disposition ||
    !['en', 'ru', 'uz'].includes(snapshot.sourceLocale) ||
    snapshot.targetLocale !== record.locale ||
    !['initial-navigation', 'locale-reload', 'capture-route-navigation'].includes(snapshot.cause) ||
    !['pre_document_commit', 'post_document_commit'].includes(snapshot.phase) ||
    snapshot.outcome !== 'declared_pending'
  )
    throw new Error(`Malformed declared pending request: ${record.cellId}`);
  if (
    record.diagnostics.requestLifecycles.some(
      (lifecycle) => lifecycle.requestId === snapshot.requestId,
    )
  )
...
export function validateDeclaredPendingRelease(release, snapshot) {
  ...
  if (
    release?.schema !== 'fe058-admission/declared-pending-release-v1' ||
    !pending ||
    !terminal ||
    pending.requestId !== snapshot.requestId ||
    pending.method !== snapshot.method ||
    pending.path !== snapshot.path ||
    pending.captureWindowId !== snapshot.captureWindowId ||
    pending.navigationEpoch !== snapshot.navigationEpoch ||
    JSON.stringify(pending.context) !== JSON.stringify(snapshot.context) ||
    pending.sourceLocale !== snapshot.sourceLocale ||
    pending.targetLocale !== snapshot.targetLocale ||
    pending.cause !== snapshot.cause ||
    pending.phase !== snapshot.phase ||
    pending.outcome !== 'declared_pending' ||
    terminal.requestId !== snapshot.requestId ||
    terminal.outcome !== 'response' ||
    terminal.status !== 204 ||
...
export function validateCatalogHeroLifecycles(record) {
  ...
  if (!Array.isArray(lifecycles))
    throw new Error(`Malformed catalog hero lifecycle diagnostics: ${record.cellId}`);
  if (!lifecycles.length) return;
  if (record.matrix !== 'M03' || record.scenario !== 'hero-price-sort' || record.route !== '/')
    throw new Error(
      `Catalog hero lifecycle leaked outside its Catalog capture window: ${record.cellId}`,
    );
  const requestIds = new Set();
  ...
  for (const event of lifecycles) {
    ...
    if (
      !Number.isInteger(event?.requestId) ||
      event.requestId < 1 ||
      ...
      event.method !== 'GET' ||
      event.path !== '/src/pages/catalog-page/assets/catalog-hero-ui025.png' ||
      event.resourceType !== 'image' ||
      !boundary ||
      !Number.isInteger(boundary.epoch) ||
      boundary.epoch < 1 ||
      !['locale-reload', 'capture-route-navigation'].includes(boundary.cause) ||
      !['en', 'ru', 'uz'].includes(boundary.sourceLocale) ||
      boundary.targetLocale !== record.locale ||
      boundary.matrix !== 'M03' ||
      boundary.scenario !== 'hero-price-sort' ||
      boundary.route !== '/'
    )
    ...
    if (event.kind === 'aborted') {
      if (
        ...
        event.replacesRequestId !== undefined ||
        event.catalogHeroRendered !== undefined ||
        boundary.phase !== 'pre_document_commit' ||
        boundary.documentCommitted !== false
      )
        throw new Error(`Unproven catalog hero abort: ${record.cellId}`);
      const replacement = lifecycles.find(
        (candidate) =>
          candidate.kind === 'replacement' && candidate.replacesRequestId === event.requestId,
      );
      if (
        !replacement ||
        replacement.requestId === event.requestId ||
        replacement.sequence <= event.sequence ||
        replacement.boundary.epoch !== boundary.epoch ||
        replacement.boundary.cause !== boundary.cause ||
        replacement.boundary.sourceLocale !== boundary.sourceLocale ||
        replacement.boundary.targetLocale !== boundary.targetLocale ||
        replacement.boundary.phase !== 'post_document_commit' ||
        replacement.boundary.documentCommitted !== true ||
        !Number.isInteger(replacement.status) ||
        replacement.status < 200 ||
        replacement.status >= 300 ||
        replacement.catalogHeroRendered !== true
      )
        throw new Error(
          `Catalog hero abort lacks a distinct rendered replacement: ${record.cellId}`,
        );
    } else if (
      event.kind !== 'replacement' ||
      !Number.isInteger(event.replacesRequestId) ||
  ...
  for (const replacement of lifecycles.filter((event) => event.kind === 'replacement'))
    if (
      !lifecycles.some(
        (event) => event.kind === 'aborted' && event.requestId === replacement.replacesRequestId,
      )
    )
      throw new Error(`Catalog hero replacement has no abort: ${record.cellId}`);
}
export function validateLocaleConvergence(record) {
  const convergence = record.localeConvergence;
  if (
    !convergence ||
    convergence.documentLocale !== record.locale ||
    convergence.storageLocale !== record.locale ||
    convergence.storagePresent !== true
  )
    throw new Error(
      `Missing or mismatched target locale convergence: ${record.cellId ?? 'M01 outcome'}`,
    );
}
export function validateAggregateRecordIdentities(record) {
  const identities = record.identities;
  if (!Array.isArray(identities)) throw new Error(`Malformed target identities: ${record.cellId}`);
  const seen = new Set();
  for (const identity of identities) {
    ...
    if (seen.has(identity.path))
      throw new Error(`Duplicate executable identity ${identity.path}: ${record.cellId}`);
  ...
  for (let index = 0; index < currentIdentities.length; index += 1) {
    ...
    if (actual.path !== expected.path)
      throw new Error(
        `Unexpected executable identity key ${actual.path}; expected ${expected.path}: ${record.cellId}`,
      );
    if (actual.sha256 !== expected.sha256)
      throw new Error(`Stale executable identity ${expected.path}: ${record.cellId}`);
  }
}
...
if (legacyM02IdentityProof) {
  ...
  if (m02Index < 0)
    throw new Error('M02 identity proof owner is absent from the executable registry.');
  if (legacyM02IdentityProof === 'missing') identities.splice(m02Index, 1);
  else if (legacyM02IdentityProof === 'extra')
    identities.push({
      path: `${evidenceRelativeRoot}/m02-catalog.spec.ts.stale.ts`,
      sha256: identities[m02Index].sha256,
    });
  else if (legacyM02IdentityProof === 'stale' || legacyM02IdentityProof === 'mismatch')
    identities[m02Index].sha256 =
      `${identities[m02Index].sha256[0] === '0' ? '1' : '0'}${identities[m02Index].sha256.slice(1)}`;
  ...
  process.stdout.write(
    `${JSON.stringify({ m02IdentityNegativeProof: legacyM02IdentityProof, rejected: true })}\n`,
  );
  process.exit(0);
}
...
if (executableIdentityProof) {
  if (executableIdentityProof === 'positive')
    validateAggregateRecordIdentities({
      cellId: 'identity-proof-positive',
      identities: currentIdentities,
    });
  else if (executableIdentityProof === 'missing-each') {
    for (let index = 0; index < currentIdentities.length; index += 1)
      expectIdentityRejection(
        currentIdentities.filter((_, candidateIndex) => candidateIndex !== index),
        `missing-${currentIdentities[index].path}`,
      );
  } else if (executableIdentityProof === 'stale-each') {
    for (let index = 0; index < currentIdentities.length; index += 1) {
      const identities = currentIdentities.map((identity) => ({ ...identity }));
      identities[index].sha256 =
        `${identities[index].sha256[0] === '0' ? '1' : '0'}${identities[index].sha256.slice(1)}`;
      expectIdentityRejection(identities, `stale-${identities[index].path}`);
    }
  } else {
    const identities = currentIdentities.map((identity) => ({ ...identity }));
    if (executableIdentityProof === 'extra')
      identities.push({ path: `${evidenceRelativeRoot}/unexpected.ts`, sha256: '0'.repeat(64) });
    else if (executableIdentityProof === 'duplicate')
      identities[identities.length - 1] = { ...identities[0] };
    ...
    else if (executableIdentityProof === 'unsupported')
      identities[0].path = `${evidenceRelativeRoot}/unsupported.md`;
  ...
  process.stdout.write(
    `${JSON.stringify({ executableIdentityProof, accepted: executableIdentityProof === 'positive', rejected: executableIdentityProof !== 'positive', identityCount: currentIdentities.length, viteConfigCovered: currentIdentities.some(({ path }) => path === 'vite.config.ts') })}\n`,
  );
  process.exit(0);
}
...
if (negativeProof) {
  const record = {
    ...
    interaction: {
      ...
      target: 'Pending clear dialog',
      focus: { status: 'pass' },
      keyboard: { status: 'pass' },
      minTarget44: { status: 'pass' },
      modal: {
        initialDialogFocus: true,
        tabFocusWithinDialog: true,
        noBackgroundFocus: true,
        enabledDialogActions: negativeProof === 'enabled-action' ? 1 : 0,
      },
    },
  };
  try {
    ...
    if (String(error).includes('unexpectedly passed')) throw error;
    process.stdout.write(
      `${JSON.stringify({ negativeProof, rejected: true, reason: String(error) })}\n`,
    );
    process.exit(0);
  }
}
...
if (declaredPendingProof) {
  ...
  const snapshot = {
    ...
    sourceLocale: 'en',
    targetLocale: 'en',
    cause: 'initial-navigation',
    phase: 'post_document_commit',
    outcome: 'declared_pending',
  };
  const record = {
    ...context,
    cellId: `declared-pending-${declaredPendingProof}`,
    locale: 'en',
    diagnostics: { requestLifecycles: [], declaredPendingRequests: [snapshot] },
  };
  const release = {
    schema: 'fe058-admission/declared-pending-release-v1',
    pending: { ...snapshot },
    terminal: { requestId: 71, outcome: 'response', status: 204 },
  };
  if (declaredPendingProof === 'undeclared') record.diagnostics.declaredPendingRequests = [];
  if (declaredPendingProof === 'extra' || declaredPendingProof === 'duplicate')
    record.diagnostics.declaredPendingRequests.push({ ...snapshot });
  ...
  if (declaredPendingProof === 'context-mismatch')
    snapshot.context = { ...context, state: 'other' };
  ...
  if (declaredPendingProof === 'terminal-before-capture')
    record.diagnostics.requestLifecycles.push({
      requestId: 71,
      method: 'DELETE',
      path: '/cart',
      captureWindowId: 12,
  ...
  if (declaredPendingProof === 'ordinary-window') {
    record.matrix = 'M07';
    record.scenario = 'return-root';
    record.route = '/cart';
    record.state = 'localized-return';
  }
  try {
    const pending = validateDeclaredPendingRequests(record);
    if (declaredPendingProof === 'no-terminal-release')
      validateDeclaredPendingRelease(undefined, pending);
    else if (declaredPendingProof === 'duplicate-release')
      throw new Error('Duplicate declared-pending terminal release evidence.');
    else validateDeclaredPendingRelease(release, pending);
    if (declaredPendingProof !== 'positive')
      throw new Error(`Declared pending proof unexpectedly passed: ${declaredPendingProof}`);
  } catch (error) {
    if (declaredPendingProof === 'positive' || String(error).includes('unexpectedly passed'))
      throw error;
    process.stdout.write(
      `${JSON.stringify({ declaredPendingProof, rejected: true, reason: String(error) })}\n`,
    );
    process.exit(0);
  }
...
const retiredSetupProof = process.env.FE058_RETIRED_SETUP_PROOF;
if (retiredSetupProof) {
  const context = {
    matrix: 'M04',
  ...
  const pair = {
    setupRequestId: 301,
    replacementRequestId: 302,
    setupSequence: 301,
    replacementSequence: 302,
    method: 'GET',
    path: '/api/auth/session',
    replacementStatus: 200,
  };
  const record = {
    ...context,
    cellId: `retired-setup-${retiredSetupProof}`,
    locale: 'ru',
    diagnostics: {
      requestLifecycles: [replacement],
      retiredSetupLifecycleEvidence: {
        setupWindow,
        reloadWindow,
        setupLifecycles: [setup],
        replacementLifecycles: [replacement],
        replacementPairs: [pair],
      },
    },
  };
  const evidence = record.diagnostics.retiredSetupLifecycleEvidence;
  if (retiredSetupProof === 'repeated-cell') record.diagnostics.requestLifecycles = [];
  if (retiredSetupProof === 'ordinary-locale-reload') {
    record.diagnostics.retiredSetupLifecycleEvidence = null;
  }
  if (retiredSetupProof === 'missing') delete record.diagnostics.retiredSetupLifecycleEvidence;
  if (retiredSetupProof === 'earlier') evidence.replacementPairs[0].replacementSequence = 300;
  if (retiredSetupProof === 'reused') evidence.replacementPairs.push({ ...pair });
  if (retiredSetupProof === 'cross-window') evidence.reloadWindow.captureWindowId = 33;
  if (retiredSetupProof === 'cross-epoch') evidence.reloadWindow.navigationEpoch = 33;
  if (retiredSetupProof === 'cross-context')
    evidence.reloadWindow.context = { ...context, state: 'other' };
  if (retiredSetupProof === 'cross-locale') evidence.reloadWindow.sourceLocale = 'ru';
  if (retiredSetupProof === 'cause-phase-drift')
    evidence.reloadWindow.phase = 'pre_document_commit';
  if (retiredSetupProof === 'write-method') evidence.setupLifecycles[0].method = 'POST';
  if (retiredSetupProof === 'non-2xx') evidence.replacementPairs[0].replacementStatus = 500;
  if (retiredSetupProof === 'unresolved') evidence.replacementPairs[0].replacementRequestId = 999;
  try {
    validateRequestLifecycles(record);
    validateRetiredSetupLifecycleEvidence(record);
    if (!['positive', 'repeated-cell', 'ordinary-locale-reload'].includes(retiredSetupProof))
      throw new Error(`Retired setup proof unexpectedly passed: ${retiredSetupProof}`);
  } catch (error) {
    if (
      ['positive', 'repeated-cell', 'ordinary-locale-reload'].includes(retiredSetupProof) ||
      String(error).includes('unexpectedly passed')
    )
      throw error;
    process.stdout.write(
      `${JSON.stringify({ retiredSetupProof, rejected: true, reason: String(error) })}\n`,
    );
    process.exit(0);
  }
  process.stdout.write(`${JSON.stringify({ retiredSetupProof, accepted: true })}\n`);
  process.exit(0);
}
...
if (supersededReadProof) {
  const context = {
    matrix: 'M06',
    scenario: 'full-page-actions',
    route: '/ai-chat',
    state: 'history-ready',
    session: 'authenticated',
    disposition: 'observed',
  };
  const failed = (requestId, sequence) => ({
    requestId,
    sequence,
    method: 'GET',
    path: '/read',
  ...
  const response = (requestId, sequence) => ({
    requestId,
    sequence,
    method: 'GET',
    path: '/read',
  ...
  const pair = (failedRequestId, replacementRequestId, failedSequence, replacementSequence) => ({
    failedRequestId,
  ...
  const record = {
    ...context,
    cellId: `superseded-read-${supersededReadProof}`,
    locale: 'en',
    diagnostics: {
      requestLifecycles: lifecycles,
      supersededReadLifecycles: [pair(81, 82, 1, 2), pair(83, 84, 3, 4)],
    },
  };
  ...
  if (supersededReadProof === 'reused-replacement')
    record.diagnostics.supersededReadLifecycles[1].replacementRequestId = 82;
  if (supersededReadProof === 'unmatched') first.replacementRequestId = 999;
  try {
    validateRequestLifecycles(record);
    validateSupersededReadLifecycles(record);
    validateRetiredSetupLifecycleEvidence(record);
    if (supersededReadProof !== 'positive')
      throw new Error(`Superseded-read proof unexpectedly passed: ${supersededReadProof}`);
  } catch (error) {
    if (supersededReadProof === 'positive' || String(error).includes('unexpectedly passed'))
      throw error;
    process.stdout.write(
      `${JSON.stringify({ supersededReadProof, rejected: true, reason: String(error) })}\n`,
    );
    process.exit(0);
  }
...
if (requestLifecycleProof) {
  ...
  const lifecycle = {
    ...
    sequence: 1,
    method: 'GET',
    path: '/courses/7/progress',
    captureWindowId: 9,
    navigationEpoch: 9,
    context,
    sourceLocale: 'en',
    targetLocale: 'en',
    cause: 'capture-route-navigation',
  ...
  const record = {
    ...context,
    cellId: `request-lifecycle-${requestLifecycleProof}`,
    locale: 'en',
    diagnostics: { requestLifecycles: [lifecycle], supersededReadLifecycles: [] },
  };
  if (requestLifecycleProof === 'duplicate-completion')
    record.diagnostics.requestLifecycles.push({ ...lifecycle });
  if (requestLifecycleProof === 'wrong-window')
    lifecycle.context = { ...context, scenario: 'full-page-menu' };
  ...
  if (requestLifecycleProof === 'open-request') {
    lifecycle.outcome = 'response';
    delete lifecycle.status;
  }
  if (requestLifecycleProof === 'real-current-window-abort') {
    lifecycle.outcome = 'failure';
    delete lifecycle.status;
    lifecycle.error = 'net::ERR_ABORTED';
  }
  try {
    validateRequestLifecycles(record);
    validateSupersededReadLifecycles(record);
    if (requestLifecycleProof !== 'positive')
      throw new Error(`Request lifecycle proof unexpectedly passed: ${requestLifecycleProof}`);
  } catch (error) {
    if (requestLifecycleProof === 'positive' || String(error).includes('unexpectedly passed'))
      throw error;
    process.stdout.write(
      `${JSON.stringify({ requestLifecycleProof, rejected: true, reason: String(error) })}\n`,
    );
    process.exit(0);
  }
...
if (requestFailureNegativeProof) {
  const sourceLocale =
    requestFailureNegativeProof === 'source-target-mismatch' ||
    requestFailureNegativeProof === 'wrong-context'
      ? 'ru'
      : 'en';
  const targetLocale =
    requestFailureNegativeProof === 'wrong-locale' ||
    requestFailureNegativeProof === 'wrong-context'
      ? 'ru'
      : 'en';
  const invalidTeardown = {
    ...(requestFailureNegativeProof === 'missing-request-id' ? {} : { requestId: 1 }),
    method: 'GET',
    path:
      requestFailureNegativeProof === 'wrong-path' ? '/courses/7/unknown' : '/courses/7/progress',
    error: requestFailureNegativeProof === 'wrong-error' ? 'net::ERR_FAILED' : 'net::ERR_ABORTED',
    boundary: {
      cause:
        requestFailureNegativeProof === 'wrong-cause'
          ? 'wrong-cause'
          : requestFailureNegativeProof === 'wrong-context'
            ? 'locale-reload'
      ...
      sourceLocale,
      targetLocale,
      epoch: requestFailureNegativeProof === 'stale-epoch' ? 0 : 7,
      phase:
        requestFailureNegativeProof === 'post-commit'
          ? 'post_document_commit'
          : 'pre_document_commit',
      matrix: 'M01',
      scenario: 'completion-ready',
      route: '/learning/enrollments/4',
    },
    preNavigationRequest: requestFailureNegativeProof !== 'missing-pre-navigation-proof',
  };
  try {
    validateNavigationTeardowns({
      cellId: `negative-${requestFailureNegativeProof}`,
      matrix: 'M01',
      scenario: 'completion-ready',
      route: '/learning/enrollments/4',
      locale: 'en',
      diagnostics: {
        navigationTeardowns:
          requestFailureNegativeProof === 'duplicate-request'
            ? [invalidTeardown, { ...invalidTeardown }]
            : [invalidTeardown],
      },
    });
    throw new Error(
      `Negative request-failure proof unexpectedly passed: ${requestFailureNegativeProof}`,
    );
  } catch (error) {
    if (String(error).includes('unexpectedly passed')) throw error;
    process.stdout.write(
      `${JSON.stringify({ requestFailureNegativeProof, rejected: true, reason: String(error) })}\n`,
    );
    process.exit(0);
  }
}
...
if (requestFailurePositiveProof) {
  const validTeardown = {
    requestId: 1,
    method: 'GET',
    path: requestFailurePositiveProof === 'lessons' ? '/courses/7/lessons' : '/courses/7/progress',
    error: 'net::ERR_ABORTED',
    boundary: {
      cause:
        requestFailurePositiveProof === 'locale-reload'
          ? 'locale-reload'
          : 'capture-route-navigation',
      sourceLocale: requestFailurePositiveProof === 'locale-reload' ? 'ru' : 'en',
      targetLocale: 'en',
      epoch: 7,
      phase: 'pre_document_commit',
      matrix: 'M01',
    ...
    preNavigationRequest: true,
  };
  validateNavigationTeardowns({
    cellId: `positive-${requestFailurePositiveProof}`,
    matrix: 'M01',
    scenario: 'completion-ready',
    route: '/learning/enrollments/4',
    locale: 'en',
    diagnostics: { navigationTeardowns: [validTeardown] },
  });
...
if (catalogHeroLifecycleProof) {
  ...
  const record = {
    cellId: `catalog-hero-${catalogHeroLifecycleProof}`,
    matrix: catalogHeroLifecycleProof === 'wrong-context' ? 'M02' : 'M03',
    scenario:
      catalogHeroLifecycleProof === 'wrong-context' ? 'course-detail-success' : 'hero-price-sort',
    route: catalogHeroLifecycleProof === 'wrong-context' ? '/courses/7' : '/',
    locale: 'en',
    diagnostics: {
      catalogHeroLifecycles:
  ...
  if (catalogHeroLifecycleProof === 'wrong-path')
    lifecycle[0].path = '/src/pages/catalog-page/assets/other.png';
  if (catalogHeroLifecycleProof === 'wrong-cause') lifecycle[0].boundary.cause = 'wrong-cause';
  if (catalogHeroLifecycleProof === 'post-commit') {
    lifecycle[0].boundary.phase = 'post_document_commit';
    lifecycle[0].boundary.documentCommitted = true;
  }
  if (catalogHeroLifecycleProof === 'stale-epoch') lifecycle[0].boundary.epoch = 0;
  if (catalogHeroLifecycleProof === 'wrong-locale')
    lifecycle.forEach((event) => {
      event.boundary.targetLocale = 'ru';
    });
  ...
  if (catalogHeroLifecycleProof === 'source-locale-mismatch')
    lifecycle[1].boundary.sourceLocale = 'ru';
  if (catalogHeroLifecycleProof === 'missing-source-locale')
    delete lifecycle[0].boundary.sourceLocale;
  if (catalogHeroLifecycleProof === 'unsupported-source-locale')
    lifecycle[0].boundary.sourceLocale = 'fr';
  try {
    validateCatalogHeroLifecycles(record);
    if (catalogHeroLifecycleProof !== 'positive')
      throw new Error(
        `Catalog hero lifecycle proof unexpectedly passed: ${catalogHeroLifecycleProof}`,
      );
  } catch (error) {
    if (catalogHeroLifecycleProof === 'positive' || String(error).includes('unexpectedly passed'))
      throw error;
    process.stdout.write(
      `${JSON.stringify({ catalogHeroLifecycleProof, rejected: true, reason: String(error) })}\n`,
    );
    process.exit(0);
  }
...
if (bootstrapProof) {
  const boundary = {
    cause:
      bootstrapProof === 'route-navigation-missing' ? 'capture-route-navigation' : 'locale-reload',
    ...
    route: '/learning/enrollments/4',
    bootstrap: {
      kind: 'initial-default-en-missing-storage',
      observedDocumentLocale: 'en',
      observedStorage: 'missing',
    },
  };
  const record = {
    cellId: `bootstrap-${bootstrapProof}`,
    matrix: 'M01',
    scenario: 'completion-ready',
    route: '/learning/enrollments/4',
    locale: boundary.targetLocale,
    localeConvergence:
      bootstrapProof === 'no-target-convergence'
        ? undefined
        : {
            documentLocale: boundary.targetLocale,
            storageLocale: bootstrapProof === 'mismatched-storage' ? 'ru' : boundary.targetLocale,
            storagePresent: true,
          },
    diagnostics: {
      navigationTeardowns: [
  ...
  try {
    ...
    if (!['initial-en', 'target-ru'].includes(bootstrapProof))
      throw new Error(`Bootstrap proof unexpectedly passed: ${bootstrapProof}`);
  } catch (error) {
    if (['initial-en', 'target-ru'].includes(bootstrapProof)) throw error;
    if (String(error).includes('unexpectedly passed')) throw error;
    process.stdout.write(
      `${JSON.stringify({ bootstrapProof, rejected: true, reason: String(error) })}\n`,
    );
    process.exit(0);
  }
...
if (terminalFamilyProof) {
  ...
  const resolvedTempRoot = resolve(tmpdir());
  if (!resolve(proofRoot).startsWith(`${resolvedTempRoot}${sep}`))
    throw new Error(`Refusing terminal proof outside the system temporary directory: ${proofRoot}`);
  try {
    const proofRunId = 'terminal-proof-run';
    const proofDirectories = expectedShardDirectoryNames(proofRunId).map((name) =>
      join(proofRoot, name),
    );
    for (const directory of proofDirectories) {
      mkdirSync(directory);
      writeFileSync(join(directory, '.last-run.json'), stableJson({ status: 'passed' }), {
        flag: 'wx',
      });
    }
    ...
    else if (terminalFamilyProof === 'failed')
      writeFileSync(
        join(candidateDirectories[5], '.last-run.json'),
        stableJson({ status: 'failed' }),
      );
    else if (terminalFamilyProof === 'malformed')
      writeFileSync(join(candidateDirectories[5], '.last-run.json'), '{');
    else if (terminalFamilyProof === 'stale-runid') expectedRunId = `${proofRunId}-stale`;
    else if (terminalFamilyProof === 'duplicate-directory')
      candidateDirectories[candidateDirectories.length - 1] = candidateDirectories[0];
    else if (!['positive', 'marker-drift'].includes(terminalFamilyProof))
      throw new Error(`Unknown terminal family proof: ${terminalFamilyProof}`);
    let rejected = false;
    try {
      ...
      if (terminalFamilyProof !== 'positive')
        throw new Error(`Terminal family proof unexpectedly passed: ${terminalFamilyProof}`);
    } catch (error) {
      if (String(error).includes('unexpectedly passed')) throw error;
    ...
    process.stdout.write(
      `${JSON.stringify({ terminalFamilyProof, accepted: terminalFamilyProof === 'positive', rejected })}\n`,
    );
    rmSync(proofRoot, { recursive: true, force: true });
    process.exit(0);
  } finally {
...
if (resultSnapshotProof) {
  ...
  if (!resolve(proofRoot).startsWith(`${resolvedTempRoot}${sep}`))
    throw new Error(
      `Refusing result snapshot proof outside the system temporary directory: ${proofRoot}`,
    );
  ...
  try {
    ...
    const proofScreenshotMode =
      resultSnapshotProof.startsWith('canonical') || resultSnapshotProof === 'wrong-mode-inventory'
        ? 'canonical'
        : 'full';
    ...
    orderedCellIds.forEach((cellId, index) => {
      ...
      writeFileSync(
        join(directory, 'proof-output', 'cells', `${cellId}.json`),
        stableJson({ cellId }),
        { flag: 'wx' },
      );
      if (
        !(
          resultSnapshotProof === 'canonical-missing-screenshot' && cellId === firstCapturedCellId
        ) &&
        proofCapturedCellIds.has(cellId)
      )
        writeFileSync(
          join(directory, 'proof-output', 'screenshots', `${cellId}.png`),
          Buffer.from(`png-${cellId}`),
          { flag: 'wx' },
        );
    });
    const outcomePath = join(proofDirectories[0], 'proof-output', 'm01-outcome.json');
    const releasePath = join(
      proofDirectories.at(-1),
      'proof-output',
      'declared-pending-release.json',
    );
    const outcomeScreenshotPath = join(
      proofDirectories[0],
      'proof-output',
      'screenshots',
      'm01-current-outcome.png',
    );
    writeFileSync(outcomePath, stableJson({ schema: 'fe058-admission/current-m01-outcome-v1' }), {
      flag: 'wx',
    });
    writeFileSync(
      releasePath,
      stableJson({ schema: 'fe058-admission/declared-pending-release-v1' }),
      { flag: 'wx' },
    );
    ...
    try {
      const candidate = {
        path: `${proofPathPrefix}/${basename(proofDirectories[0])}/proof-output/cells/probe.json`,
        role: 'cell-result',
        bytes: Buffer.from('{}'),
      };
      if (
        resultSnapshotProof === 'canonical-extra-full-screenshot' ||
        resultSnapshotProof === 'wrong-mode-inventory'
      ) {
        const extraCellId = orderedCellIds.find((cellId) => !proofCapturedCellIds.has(cellId));
        writeFileSync(
          join(proofDirectories[0], 'proof-output', 'screenshots', `${extraCellId}.png`),
          Buffer.from(`png-${extraCellId}`),
          { flag: 'wx' },
        );
      }
      snapshot = captureCanonicalResultSnapshot(proofDirectories, proofRunId, {
        resultsRoot: proofRoot,
        pathPrefix: proofPathPrefix,
        expectedCapturedCells: proofCapturedCellIds,
      });
      if (['canonical-positive', 'full-positive', 'positive'].includes(resultSnapshotProof)) {
        ...
        publishAcceptedOutputs({
          ...
          terminalRecord: {
            ...
            runId: proofRunId,
            status: 'complete',
            report: { resultSnapshot: snapshot.contract },
          },
          ...
          buildManifest: ({ terminalSha256, reportSha256 }) => ({
            ...
            terminal: { sha256: terminalSha256 },
            close_or_route: { sha256: reportSha256 },
            result_snapshot: resultSnapshotManifestBinding(snapshot.contract, proofPathPrefix),
          }),
          generationId,
      ...
      } else if (resultSnapshotProof === 'mutation')
        writeFileSync(outcomePath, stableJson({ mutated: true }));
      else if (resultSnapshotProof === 'deletion') rmSync(releasePath);
      else if (resultSnapshotProof === 'addition')
        writeFileSync(join(proofDirectories[0], 'proof-output', 'cells', 'extra.json'), '{}', {
          flag: 'wx',
        });
      else if (resultSnapshotProof === 'screenshot-drift')
        writeFileSync(outcomeScreenshotPath, 'drifted-screenshot');
      else if (resultSnapshotProof === 'result-drift')
        writeFileSync(
          join(proofDirectories[0], 'proof-output', 'cells', `${orderedCellIds[0]}.json`),
          '{"drift":true}',
        );
      else if (resultSnapshotProof === 'marker-drift')
        writeFileSync(
          join(proofDirectories[0], '.last-run.json'),
          stableJson({ status: 'passed', drift: true }),
        );
      else if (resultSnapshotProof === 'unsupported-extension')
        writeFileSync(join(proofDirectories[0], 'proof-output', 'unsupported.txt'), 'unsupported', {
          flag: 'wx',
        });
      else if (resultSnapshotProof === 'alias')
        buildResultSnapshotContract(
          [
            {
              ...candidate,
              path: `${proofPathPrefix}/${basename(proofDirectories[0])}/proof-output/./cells/probe.json`,
      ...
      else if (resultSnapshotProof === 'case-collision')
        buildResultSnapshotContract(
          [candidate, { ...candidate, path: candidate.path.replace('/probe.json', '/PROBE.json') }],
          proofPathPrefix,
        );
      else if (resultSnapshotProof === 'path-escape')
        buildResultSnapshotContract(
          [{ ...candidate, path: `${proofPathPrefix}/../escape.json` }],
          proofPathPrefix,
        );
      else if (resultSnapshotProof === 'duplicate')
        buildResultSnapshotContract([candidate, { ...candidate }], proofPathPrefix);
      else if (resultSnapshotProof === 'unsupported-role')
        buildResultSnapshotContract(
          [{ ...candidate, role: 'unknown-result-role' }],
          proofPathPrefix,
        );
      else if (
        ![
          'canonical-missing-screenshot',
          'canonical-extra-full-screenshot',
          'wrong-mode-inventory',
        ].includes(resultSnapshotProof)
      )
        throw new Error(`Unknown result snapshot proof: ${resultSnapshotProof}`);
      if (
        [
          'mutation',
          'deletion',
      ...
      if (!['positive', 'canonical-positive', 'full-positive'].includes(resultSnapshotProof))
        throw new Error(`Result snapshot proof unexpectedly passed: ${resultSnapshotProof}`);
    } catch (error) {
      if (
        String(error).includes('unexpectedly passed') ||
        String(error).includes('Unknown result snapshot proof')
      )
        throw error;
      if (['positive', 'canonical-positive', 'full-positive'].includes(resultSnapshotProof))
        throw error;
    ...
    process.stdout.write(
      `${JSON.stringify({ resultSnapshotProof, accepted: ['positive', 'canonical-positive', 'full-positive'].includes(resultSnapshotProof), rejected, count: snapshot?.contract.count, digest: snapshot?.contract.digest, reason })}\n`,
    );
    rmSync(proofRoot, { recursive: true, force: true });
    process.exit(0);
  } finally {
...
if (snapshotDigestProof) {
  ...
  const proofEntries = [
    {
      path: `${proofPathPrefix}/alpha.json`,
      size: 0,
      sha256: sha256(Buffer.alloc(0)),
      role: 'terminal-marker',
    },
    {
      path: `${proofPathPrefix}/nested/result.json`,
      size: 7,
      sha256: sha256(Buffer.from('result\n')),
      role: 'cell-result',
    },
    {
      path: `${proofPathPrefix}/résumé-uz.png`,
      size: 3,
      sha256: sha256(Buffer.from('png')),
      role: 'screenshot',
    },
  ].sort((left, right) => compareCanonicalPaths(left.path, right.path));
  const baseline = computeCanonicalResultSnapshotDigest(
    resultSnapshotSchema,
    proofEntries.length,
    proofEntries,
    proofPathPrefix,
  ...
  if (snapshotDigestProof === 'positive') {
    ...
    output.accepted =
      independent.digest === baseline.digest &&
      independent.preimageByteLength === baseline.preimageByteLength;
  } else if (snapshotDigestProof === 'publication') {
    const publicationEntries = proofEntries.map((entry) => ({
      ...entry,
      path: `${evidenceRelativeRoot}/results/${entry.path}`,
    }));
    const publicationDigest = computeCanonicalResultSnapshotDigest(
      resultSnapshotSchema,
      publicationEntries.length,
      publicationEntries,
      `${evidenceRelativeRoot}/results`,
    );
    const contract = {
      schema: resultSnapshotSchema,
      digestAlgorithm: resultSnapshotDigestAlgorithm,
    ...
    try {
      ...
      const published = publishAcceptedOutputs({
        ...
        terminalRecord: {
          ...
          runId: 'proof-run',
          status: 'complete',
          report: { resultSnapshot: contract },
        },
        reportMarkdown: '# canonical snapshot digest publication proof\n',
        buildManifest: ({ terminalSha256, reportSha256 }) => ({
          schema: 'fe058-admission/v5',
          generation_id: generationId,
          result_snapshot: resultSnapshotManifestBinding(
            contract,
            `${evidenceRelativeRoot}/results`,
          ),
          terminal: { sha256: terminalSha256 },
    ...
    const mutations = [
      [
        ...
        proofEntries.length,
        proofEntries,
        algorithmMutation({ id: `${resultSnapshotDigestAlgorithm.id}-changed` }),
      ],
      [
        'algorithm-version',
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries,
        algorithmMutation({ version: resultSnapshotDigestAlgorithm.version + 1 }),
      ],
      [
        ...
        proofEntries.length,
        proofEntries,
        algorithmMutation({ roleEnumeration: [...resultSnapshotRoleEnumeration].reverse() }),
      ],
      [
        'role-enumeration-value',
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries,
        algorithmMutation({
          roleEnumeration: resultSnapshotRoleEnumeration.map((role, index) =>
            index ? role : 'changed-terminal-marker',
          ),
        }),
      ],
      [
        'role-enumeration-add',
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries,
        algorithmMutation({ roleEnumeration: [...resultSnapshotRoleEnumeration, 'added-role'] }),
      ],
      [
        'role-enumeration-remove',
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries,
        algorithmMutation({ roleEnumeration: resultSnapshotRoleEnumeration.slice(0, -1) }),
      ],
      [
        'algorithm-delimiter',
        resultSnapshotSchema,
      ...
      [
        ...
        proofEntries.length,
        proofEntries,
        algorithmMutation({ id: `${resultSnapshotDigestAlgorithm.id}\nchanged` }),
      ],
      [
        'algorithm-non-nfc',
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries,
        algorithmMutation({ id: 'fe058-re\u0301sume\u0301' }),
      ],
      [
        'role-enumeration-delimiter',
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries,
        algorithmMutation({
          roleEnumeration: resultSnapshotRoleEnumeration.map((role, index) =>
            index ? role : 'terminal\tmarker',
          ),
        }),
      ],
      [
        'role-enumeration-control',
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries,
        algorithmMutation({
      ...
      [
        'role-enumeration-non-nfc',
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries,
        algorithmMutation({
          roleEnumeration: resultSnapshotRoleEnumeration.map((role, index) =>
            index ? role : 're\u0301sume\u0301-role',
          ),
        }),
      ],
      [
        'schema',
        `${resultSnapshotSchema}-changed`,
        proofEntries.length,
        proofEntries,
        resultSnapshotDigestAlgorithm,
      ],
      [
        'count',
        resultSnapshotSchema,
      ...
      [
        ...
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries.map((entry, index) => (index ? entry : { ...entry, size: entry.size + 1 })),
        resultSnapshotDigestAlgorithm,
      ],
      [
        'missing-field',
        ...
        proofEntries.map((entry, index) =>
          index ? entry : { path: entry.path, size: entry.size, role: entry.role },
        ),
        resultSnapshotDigestAlgorithm,
      ],
      [
        ...
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries.map((entry, index) => (index ? entry : { ...entry, unsupported: true })),
        resultSnapshotDigestAlgorithm,
      ],
      [
        'entry-delimiter',
        ...
        proofEntries.length,
        proofEntries.map((entry, index) =>
          index ? entry : { ...entry, path: `${proofPathPrefix}/bad\tpath.json` },
        ),
        resultSnapshotDigestAlgorithm,
      ],
      [
        ...
        proofEntries.length,
        proofEntries.map((entry, index) =>
          index ? entry : { ...entry, path: `${proofPathPrefix}/bad\npath.json` },
        ),
        resultSnapshotDigestAlgorithm,
      ],
      [
        ...
        proofEntries.length,
        proofEntries.map((entry, index) =>
          index ? entry : { ...entry, path: `${proofPathPrefix}/re\u0301sume\u0301.json` },
        ),
        resultSnapshotDigestAlgorithm,
      ],
      [
        ...
        proofEntries.length,
        proofEntries.map((entry, index) =>
          index ? entry : { ...entry, path: `${proofPathPrefix}/../escape.json` },
        ),
        resultSnapshotDigestAlgorithm,
      ],
      [
        ...
        resultSnapshotSchema,
        proofEntries.length,
        proofEntries.map((entry, index) => (index ? entry : { ...entry, size: -1 })),
        resultSnapshotDigestAlgorithm,
      ],
      [
        'hash',
        ...
        proofEntries.length,
        proofEntries.map((entry, index) =>
          index ? entry : { ...entry, sha256: entry.sha256.toUpperCase() },
        ),
        resultSnapshotDigestAlgorithm,
      ],
      [
        ...
        proofEntries.length,
        proofEntries.map((entry, index) =>
          index ? entry : { ...entry, role: 'unsupported-role' },
        ),
        resultSnapshotDigestAlgorithm,
      ],
    ];
    output.mutations = mutations.map(([name, schema, count, entries, digestAlgorithm]) => {
      try {
        const mutated = computeCanonicalResultSnapshotDigest(
          schema,
          count,
          entries,
          proofPathPrefix,
        ...
        return {
          ...
          rejected: false,
          digestChanged:
            mutated.digest !== baseline.digest ||
            mutated.preimageByteLength !== baseline.preimageByteLength,
        };
      } catch (error) {
        return { name, rejected: true, digestChanged: false, reason: String(error) };
      }
    });
    const baselinePreimage = encodeCanonicalResultSnapshotPreimage(
      resultSnapshotSchema,
      proofEntries.length,
      proofEntries,
      proofPathPrefix,
    ...
    const rawHeaderMutations = [
      ...
      [
        'role-enumeration-order',
        'roles',
        `roles\t${[...resultSnapshotRoleEnumeration].reverse().join('\t')}`,
      ],
      [
        'role-enumeration-value',
        'roles',
        `roles\tchanged-terminal-marker\t${resultSnapshotRoleEnumeration.slice(1).join('\t')}`,
      ],
      [
        'role-enumeration-add',
        'roles',
        `roles\t${[...resultSnapshotRoleEnumeration, 'added-role'].join('\t')}`,
      ],
      [
        'role-enumeration-remove',
        'roles',
        `roles\t${resultSnapshotRoleEnumeration.slice(0, -1).join('\t')}`,
      ],
    ...
    if (!inputPath)
      throw new Error('FE058_SNAPSHOT_DIGEST_INPUT_TERMINAL is required for the R15 digest proof.');
    const terminalSnapshot = JSON.parse(readFileSync(inputPath, 'utf8')).report?.resultSnapshot;
    if (
      !terminalSnapshot ||
      terminalSnapshot.schema !== 'fe058-admission/canonical-result-snapshot-v1' ||
      terminalSnapshot.count !== 2090 ||
      !Array.isArray(terminalSnapshot.entries)
    )
      throw new Error(
        'R15 digest proof requires the exact historical 2,090-entry v1 terminal snapshot.',
      );
    const currentPathPrefix = `${evidenceRelativeRoot}/results`;
    validateCanonicalResultSnapshotEntries(
      resultSnapshotSchema,
      terminalSnapshot.count,
      terminalSnapshot.entries,
      currentPathPrefix,
    );
    for (const entry of terminalSnapshot.entries) {
      ...
      if (bytes.length !== entry.size || sha256(bytes) !== entry.sha256)
        throw new Error(`R15 snapshot entry bytes drifted: ${entry.path}`);
    }
    const oldInsertionOrderPayload = {
      schema: terminalSnapshot.schema,
      count: terminalSnapshot.count,
      entries: terminalSnapshot.entries,
    };
    const oldReorderedPayload = {
      entries: terminalSnapshot.entries.map(({ path, size, sha256: entrySha256, role }) => ({
        role,
        sha256: entrySha256,
        size,
        path,
    ...
    const canonical = computeCanonicalResultSnapshotDigest(
      resultSnapshotSchema,
      terminalSnapshot.count,
      terminalSnapshot.entries,
      currentPathPrefix,
    ...
    const preCorrectionDigestAlgorithm = Object.fromEntries(
      Object.entries(resultSnapshotDigestAlgorithm).filter(([key]) => key !== 'roleEnumeration'),
    );
...
if (!directories.length)
  throw new Error('Provide only terminal result directories to aggregate-v2.mjs.');
...
for (const entry of resultSnapshot.contract.entries) {
  if (entry.role === 'cell-result') {
    ...
    if (record.disposition !== 'observed')
      throw new Error(
        `Unexpected product/runtime disposition: ${record.cellId} ${record.disposition}`,
      );
    validateLocaleConvergence(record);
    validateInteractionRecord(record);
    if (
      !Array.isArray(record.diagnostics.consoleErrors) ||
      !Array.isArray(record.diagnostics.pageErrors) ||
      !Array.isArray(record.diagnostics.requestFailures) ||
      !Array.isArray(record.diagnostics.requestLifecycles) ||
      !Array.isArray(record.diagnostics.supersededReadLifecycles) ||
      !Array.isArray(record.diagnostics.declaredPendingRequests) ||
      !Array.isArray(record.diagnostics.catalogHeroLifecycles) ||
      !Array.isArray(record.diagnostics.httpErrors) ||
      !Array.isArray(record.diagnostics.writes)
    )
    ...
    validateCatalogHeroLifecycles(record);
    if (
      record.diagnostics.consoleErrors.length ||
      record.diagnostics.pageErrors.length ||
      record.diagnostics.httpErrors.length
    )
      throw new Error(`Unexplained runtime error: ${record.cellId}`);
    for (const failure of record.diagnostics.requestFailures)
      if (!allowedRequestFailures.has(failure))
        throw new Error(`Unexplained request failure: ${record.cellId} ${failure}`);
    ...
    if (screenshotExpected) {
      if (
        ...
        throw new Error(`Missing required screenshot: ${record.cellId}`);
      const screenshotEntry = resolveCanonicalScreenshot(
        resultSnapshot,
        dirname(dirname(cellPath)),
        record.screenshot.path,
      );
      if (screenshotEntry.sha256 !== record.screenshot.sha256)
        throw new Error(`Unbound screenshot: ${entry.path}`);
      referencedScreenshotPaths.add(screenshotEntry.path);
      capturedCellIds.add(record.cellId);
    } else if (
      record.screenshot.kind !== 'not_captured' ||
      record.screenshot.provenance !== 'default-canonical-subset' ||
      Object.keys(record.screenshot).length !== 2
    )
      throw new Error(`Invalid non-captured screenshot provenance: ${record.cellId}`);
    validateAggregateRecordIdentities(record);
    records.push(record);
  }
  if (entry.role === 'm01-outcome')
    m01OutcomePaths.push(absoluteCanonicalResultPath(resultSnapshot, entry.path));
  if (entry.role === 'declared-pending-release')
    declaredPendingReleasePaths.push(absoluteCanonicalResultPath(resultSnapshot, entry.path));
}
const unique = new Set(records.map(({ cellId }) => cellId));
if (unique.size !== records.length)
  throw new Error('Duplicate v2 cell IDs across terminal shards.');
...
if (unknown.length || missing.length || records.length !== expectedCellIds.size)
  throw new Error(
    `Cell inventory mismatch: expected=${expectedCellIds.size} observed=${records.length} unknown=${unknown.join(',')} missing=${missing.join(',')}`,
  );
if (
  capturedCellIds.size !== expectedCapturedCellIds.size ||
  [...expectedCapturedCellIds].some((cellId) => !capturedCellIds.has(cellId))
)
  throw new Error(
    `Screenshot cell inventory mismatch for ${screenshotMode}: expected=${expectedCapturedCellIds.size} observed=${capturedCellIds.size}`,
  );
if (declaredPendingSnapshots.length !== 12)
  throw new Error(
    `Expected 12 M08 declared-pending snapshots, found ${declaredPendingSnapshots.length}.`,
  );
const declaredPending = declaredPendingSnapshots[0];
for (const snapshot of declaredPendingSnapshots)
  if (
    snapshot.requestId !== declaredPending.requestId ||
    snapshot.method !== declaredPending.method ||
    snapshot.path !== declaredPending.path ||
    snapshot.captureWindowId !== declaredPending.captureWindowId ||
    snapshot.navigationEpoch !== declaredPending.navigationEpoch ||
    JSON.stringify(snapshot.context) !== JSON.stringify(declaredPending.context) ||
    snapshot.sourceLocale !== declaredPending.sourceLocale ||
    snapshot.targetLocale !== declaredPending.targetLocale ||
    snapshot.cause !== declaredPending.cause ||
    snapshot.phase !== declaredPending.phase ||
    snapshot.outcome !== 'declared_pending'
  )
    throw new Error(
      'Declared-pending snapshots do not prove one stable request across the M08 matrix.',
    );
if (declaredPendingReleasePaths.length !== 1)
  throw new Error(
    `Expected one declared-pending terminal release, found ${declaredPendingReleasePaths.length}.`,
  );
validateDeclaredPendingRelease(
  parseCanonicalResultJson(
    resultSnapshot,
    declaredPendingReleasePaths[0],
    'declared-pending-release',
  ),
  declaredPending,
);
if (m01OutcomePaths.length !== 1)
  throw new Error(`Expected exactly one current M01 outcome, found ${m01OutcomePaths.length}.`);
...
for (const [key, value] of Object.entries({
  schema: 'fe058-admission/current-m01-outcome-v1',
  matrix: 'M01',
  scenario: 'completion-ready',
  trigger: 'Space',
}))
  if (m01Outcome[key] !== value)
    throw new Error(`Unexpected current M01 outcome ${key}: ${m01Outcome[key]}`);
if (!['normal-transition', 'accepted-routed-failure'].includes(m01Outcome.outcome))
  throw new Error(`Unexpected current M01 product disposition: ${m01Outcome.outcome}`);
if (
  !m01Outcome.utc ||
  !m01Outcome.screenshot?.path ||
  !m01Outcome.screenshot?.sha256 ||
  !m01Outcome.diagnostics ||
  !m01Outcome.identities
)
  throw new Error('Malformed current M01 outcome.');
const m01OutcomeScreenshotEntry = resolveCanonicalScreenshot(
  resultSnapshot,
  dirname(m01OutcomePath),
  m01Outcome.screenshot.path,
);
if (m01OutcomeScreenshotEntry.sha256 !== m01Outcome.screenshot.sha256)
  throw new Error('Unbound current M01 outcome screenshot.');
referencedScreenshotPaths.add(m01OutcomeScreenshotEntry.path);
if (
  !Array.isArray(m01Outcome.diagnostics.consoleErrors) ||
  !Array.isArray(m01Outcome.diagnostics.pageErrors) ||
  !Array.isArray(m01Outcome.diagnostics.requestFailures) ||
  !Array.isArray(m01Outcome.diagnostics.httpErrors) ||
  !Array.isArray(m01Outcome.diagnostics.writes) ||
  m01Outcome.diagnostics.consoleErrors.length ||
  m01Outcome.diagnostics.pageErrors.length ||
  m01Outcome.diagnostics.httpErrors.length
)
...
for (const failure of m01Outcome.diagnostics.requestFailures)
  if (!allowedRequestFailures.has(failure))
    throw new Error(`Unexplained current M01 outcome request failure: ${failure}`);
...
const capturedScreenshots = resultSnapshot.contract.entries.filter(
  ({ role }) => role === 'screenshot',
);
if (
  referencedScreenshotPaths.size !== capturedScreenshots.length ||
  capturedScreenshots.some(({ path }) => !referencedScreenshotPaths.has(path))
)
  throw new Error('Canonical result snapshot contains an unreferenced or duplicate screenshot.');
const interaction = records.reduce(
  (totals, record) => {
    for (const name of ['focus', 'keyboard', 'minTarget44'])
      totals[record.interaction[name].status] += 1;
    return totals;
  },
  { pass: 0, fail: 0, not_applicable: 0 },
);
const acceptedRouteBasis = {
  fingerprint: 'm01-keyboard-undo-no-transition',
  routedTo: 'FE-059',
  decisionIds: [
    'FE058-D05-KEYBOARD-UNDO-ROUTE-TO-FE059',
    'FE059-D01-KEYBOARD-UNDO-SCOPE-EXPANSION',
  ],
  provenance: ['R19', 'R20', 'R21', 'R22-focused'],
};
const assertAdmissionInputsCurrent = () => {
  const refreshedIdentities = readCurrentExecutableIdentities(targetFiles);
  if (JSON.stringify(refreshedIdentities) !== JSON.stringify(currentIdentities))
    throw new Error('Executable identity bytes drifted during aggregation.');
  assertCanonicalResultSnapshotCurrent(resultSnapshot, directories, runId);
};
assertAdmissionInputsCurrent();
if (!terminalPath)
  throw new Error('FE058_AGGREGATE_TERMINAL_PATH is required for canonical output publication.');
const expectedAggregateTerminalPath = resolve(
  resultsRoot,
  `runner-${runId}`,
  'aggregate-terminal.json',
);
if (resolve(terminalPath) !== expectedAggregateTerminalPath)
  throw new Error(
    `Aggregate terminal path is not the exact RunId-local runner terminal: ${terminalPath}`,
  );
...
const matrixDispositions = [
  {
    matrix: 'M01',
    disposition: 'routed_to_FE-059',
    downstreamRoute: 'FE-059',
    basis: acceptedRouteBasis.fingerprint,
  },
  ...['M02', 'M03', 'M04', 'M05', 'M06', 'M07', 'M08'].map((matrix) => ({
    matrix,
    disposition: 'closed_no_route',
...
publishAcceptedOutputs({
  ...
  buildManifest: ({ terminalSha256, reportSha256 }) => ({
    ...
    result_snapshot: resultSnapshotManifestBinding(
      resultSnapshot.contract,
      resultSnapshot.options.pathPrefix,
    ),
    terminal_shards: portableTerminalShards,
    aggregate: {
      cells: records.length,
      screenshot_mode: screenshotMode,
      screenshot_cells: capturedCellIds.size,
