...
function validateCandidate(unit, locale, violations) {
  ...
  if (!same(placeholders(candidate.candidate), [...unit.placeholdersByLocale[locale]].sort()))
  ...
  for (const locale of LOCALES) {
    ...
    for (const value of Object.values(forms)) {
      if (
        ...
        !same(placeholders(value), [...unit.placeholdersByLocale[locale]].sort())
      ) {
...
export function serializeGeneratedResources(corpus) {
  return `// Generated by localization:sync. Do not edit.\nimport type { Resource } from 'i18next';\n\nexport const GENERATED_LOCALE_RESOURCES = ${JSON.stringify(generateResources(corpus), null, 2)} as Resource;\n`;
}
...
export async function retiredConsumerViolations(corpus, sourceRoot = resolve('src')) {
  ...
  for (const unit of retired)
    for (const [file, text] of contents)
      if (
        text.includes(`${unit.namespace}:${unit.key}`) ||
        new RegExp(`['"]${unit.key}['"]|\\b${unit.key}\\b`).test(text)
      )
