...
export type AdmissionScreenshotEvidence =
  | AdmissionCapturedScreenshotEvidence
  | AdmissionNotCapturedScreenshotEvidence;
const selectedAdmissionCell = process.env.VISUAL_ADMISSION_SCREENSHOT;
const screenshotMode: AdmissionScreenshotMode =
  process.env.VISUAL_ADMISSION_SCREENSHOT_MODE === 'full' ? 'full' : 'canonical';
const selectedLocale = selectedAdmissionCell?.split('--')[2];
export const selectedAdmissionLocales: readonly AdmissionLocale[] =
  selectedLocale && admissionLocales.includes(selectedLocale as AdmissionLocale)
    ? [selectedLocale as AdmissionLocale]
    : admissionLocales;
...
export type AdmissionRequestCause =
  | 'initial-navigation'
  | 'locale-reload'
...
/** Immutable terminal setup evidence retained across one locale-reload rollover. */
export interface AdmissionRetiredSetupLifecycleEvidence {
  readonly setupWindow: AdmissionCaptureWindow;
  readonly reloadWindow: AdmissionCaptureWindow;
  readonly setupLifecycles: readonly AdmissionRequestLifecycle[];
  readonly replacementLifecycles: readonly AdmissionRequestLifecycle[];
  readonly replacementPairs: readonly AdmissionRetiredSetupReplacementPair[];
}

export interface AdmissionRetiredSetupReplacementPair {
  readonly setupRequestId: number;
  readonly replacementRequestId: number;
  readonly setupSequence: number;
  readonly replacementSequence: number;
  readonly method: 'GET';
  readonly path: string;
  readonly replacementStatus: number;
...
export type AdmissionReloadReadiness = () => Promise<unknown>;
...
export interface CatalogHeroLifecycleEvent {
  ...
  readonly sequence: number;
  readonly method: 'GET';
  readonly path: typeof catalogHeroAssetPath;
  readonly resourceType: 'image';
  readonly kind: 'aborted' | 'replacement';
  readonly status?: number;
  readonly error?: 'net::ERR_ABORTED';
  readonly replacesRequestId?: number;
  readonly catalogHeroRendered?: true;
  readonly boundary: {
    readonly epoch: number;
    readonly cause: 'locale-reload' | 'capture-route-navigation';
    readonly phase: 'pre_document_commit' | 'post_document_commit';
    readonly documentCommitted: boolean;
    readonly sourceLocale: AdmissionLocale;
    readonly targetLocale: AdmissionLocale;
    readonly matrix: 'M03';
    readonly scenario: 'hero-price-sort';
    readonly route: '/';
  };
}
interface CatalogHeroBoundary {
  readonly epoch: number;
  readonly cause: 'locale-reload' | 'capture-route-navigation';
  readonly sourceLocale: AdmissionLocale;
  readonly targetLocale: AdmissionLocale;
  phase: 'pre_document_commit' | 'post_document_commit';
...
export class NavigationTeardownEpochTracker {
  ...
  begin(
    cause: AdmissionNavigationBoundaryProvenance['cause'],
    source: LocaleReloadSource,
...
const executableIdentityRegistryPath =
  'tests/browser/visual-admission/executable-identity-registry.json';
...
function validateExecutableIdentityPath(repositoryRoot: string, path: string) {
  if (
    ...
    isAbsolute(path) ||
    path.includes('\\') ||
    normalize(path).split('\\').join('/') !== path ||
...
function parseExecutableIdentityRegistry(value: unknown): ExecutableIdentityRegistryDocument {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error('Malformed executable identity registry document.');
  const document = value as Partial<ExecutableIdentityRegistryDocument>;
  if (
    document.schema !== 'fe058-admission/executable-identity-registry-v1' ||
    !Array.isArray(document.paths) ||
    document.paths.some((path) => typeof path !== 'string')
  )
...
async function readExecutableIdentities(
...
): Promise<readonly AdmissionExecutableIdentity[]> {
  const registry = parseExecutableIdentityRegistry(
    JSON.parse(
      await readFile(join(repositoryRoot, executableIdentityRegistryPath), 'utf8'),
    ) as unknown,
  );
  ...
  assertPublicCommandBinding(
    JSON.parse(await readFile(join(repositoryRoot, publicCommandManifestPath), 'utf8')) as unknown,
  );
  return Promise.all(
    registry.paths.map(async (path) => ({
      path,
      sha256: await fileHash(join(repositoryRoot, path)),
    })),
  );
}
...
export class AdmissionRecorder {
  ...
  private readonly retiredSetupEvidence = new Map<number, AdmissionRetiredSetupLifecycleEvidence>();
  ...
  private replacementCaptureWindow: AdmissionCaptureWindow | undefined;
  ...
  private readonly onRequest = (request: Request) => {
    ...
    const captureWindow = this.replacementCaptureWindow ?? this.activeCaptureWindow;
    if (!captureWindow)
      throw new Error(
        `Request observed without an active capture window: ${request.method()} ${url.pathname}`,
      );
    const pending = {
    ...
    if (declaration) {
      if (
        declaration.request ||
        declaration.declaration.method !== pending.method ||
        declaration.declaration.path !== pending.path
      )
        throw new Error(
          `Declared pending request does not uniquely match the started request: ${pending.method} ${pending.path}`,
        );
    ...
    if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(request.method()))
      this.writes.push(`${request.method()} ${url.pathname}`);
  };
  private readonly onFrameNavigated = (frame: ReturnType<Page['mainFrame']>) => {
    if (frame === this.page.mainFrame()) {
      ...
      if (this.replacementCaptureWindow)
        this.replacementCaptureWindow.phase = 'post_document_commit';
      this.navigationEpochs.commitMainFrameDocument();
      if (this.catalogHeroBoundary) {
        this.catalogHeroBoundary.phase = 'post_document_commit';
        this.catalogHeroBoundary.documentCommitted = true;
      }
    }
  };
  private readonly onRequestFinished = (request: Request) => {
    ...
    if (
      pending &&
      !this.requestLifecycles.some((lifecycle) => lifecycle.requestId === pending.requestId)
    )
      this.recordRequestLifecycle(pending, 'response');
  ...
  private readonly onRequestFailed = (request: Request) => {
    ...
    const priorTerminal =
      pending &&
      this.requestLifecycles.find((lifecycle) => lifecycle.requestId === pending.requestId);
    ...
    if (
      pending &&
      candidate &&
      pending.captureWindow.context.matrix === 'M01' &&
      pending.captureWindow.context.scenario === 'completion-ready' &&
      pending.captureWindow.context.route === '/learning/enrollments/4' &&
      pending.method === 'GET' &&
      candidate.allowedPaths.has(pending.path) &&
      error === 'net::ERR_ABORTED'
    ) {
      this.navigationTeardowns.push({
        ...
        boundary: {
          ...
          targetLocale: candidate.sourceLocale,
          matrix: 'M01',
          scenario: 'completion-ready',
          route: '/learning/enrollments/4',
          ...(candidate.bootstrap ? { bootstrap: candidate.bootstrap } : {}),
        },
    ...
    if (
      pending &&
      this.catalogHeroBoundary &&
      pending.method === 'GET' &&
      pending.path === catalogHeroAssetPath &&
      request.resourceType() === 'image' &&
      error === 'net::ERR_ABORTED' &&
      this.catalogHeroBoundary.phase === 'pre_document_commit'
    ) {
      const boundary = this.catalogHeroBoundary;
      this.catalogHeroLifecycles.push({
        requestId: pending.requestId,
        sequence: this.lifecycleSequence++,
        method: 'GET',
        path: catalogHeroAssetPath,
        resourceType: 'image',
        kind: 'aborted',
        error: 'net::ERR_ABORTED',
        boundary: {
          ...
          documentCommitted: boundary.documentCommitted,
          sourceLocale: boundary.sourceLocale,
          targetLocale: boundary.targetLocale,
          matrix: 'M03',
    ...
    if (!pending)
      throw new Error(
        `Failed request lost its lifecycle identity: ${request.method()} ${new URL(request.url()).pathname}`,
      );
  ...
  private readonly onResponse = (response: {
    ...
    if (
      ...
      response.status() >= 200 &&
      response.status() < 300 &&
      boundary.phase === 'post_document_commit'
    ) {
      const aborted = [...this.catalogHeroLifecycles]
        .reverse()
        .find((event) => event.kind === 'aborted' && event.boundary.epoch === boundary.epoch);
      if (aborted)
        this.catalogHeroLifecycles.push({
          requestId: pending.requestId,
          sequence: this.lifecycleSequence++,
          ...
          replacesRequestId: aborted.requestId,
          boundary: {
            epoch: boundary.epoch,
            cause: boundary.cause,
            phase: boundary.phase,
  ...
  private recordRequestLifecycle(
    pending: PendingAdmissionRequest,
    outcome: AdmissionRequestLifecycle['outcome'],
    terminal: Pick<AdmissionRequestLifecycle, 'status' | 'error'> = {},
  ) {
    const window = pending.captureWindow;
    const prior = this.requestLifecycles.find(
      (lifecycle) => lifecycle.requestId === pending.requestId,
    );
    if (prior) {
      if (
        prior.outcome === outcome &&
        prior.status === terminal.status &&
        prior.error === terminal.error
      )
        return;
      throw new Error(`Request reached conflicting terminal outcomes: ${pending.requestId}`);
    }
    this.requestLifecycles.push({
      requestId: pending.requestId,
      sequence: this.lifecycleSequence++,
      method: pending.method,
      path: pending.path,
      captureWindowId: window.captureWindowId,
      navigationEpoch: window.navigationEpoch,
      context: window.context,
      sourceLocale: window.sourceLocale,
      targetLocale: window.targetLocale,
      cause: window.cause,
      phase: window.phase,
      outcome,
      ...terminal,
  ...
  private terminalPendingRequest(request: Request) {
    ...
    const declaration = [...this.declaredPendingRequests.values()].find(
      (candidate) => candidate.closed && candidate.requestObject === request,
    );
    if (declaration?.request) return declaration.request;
    throw new Error(
      `Terminal request observed without the exact closed declared-pending identity: ${request.method()} ${new URL(request.url()).pathname}`,
    );
  }

  async beginCaptureWindow(
    context: AdmissionContext,
    targetLocale: AdmissionLocale,
    cause: AdmissionRequestCause = 'initial-navigation',
  ) {
    if (this.replacementCaptureWindow)
      throw new Error('Cannot begin a capture window while an atomic rollover is active.');
    const unresolved = [...this.pendingRequests.values()].filter(
      (request) =>
        request.captureWindow.captureWindowId === this.activeCaptureWindow?.captureWindowId,
    );
    if (unresolved.length)
      throw new Error(
        `Cannot replace capture window with unresolved request(s): ${unresolved.map((request) => request.requestId).join(',')}`,
      );
    const sourceLocale = await this.observedLocale().catch(() => targetLocale);
    this.activeCaptureWindow = {
      captureWindowId: this.nextCaptureWindowId++,
      navigationEpoch: this.navigationEpoch + 1,
      context,
      sourceLocale,
      targetLocale,
      cause,
  ...
  async performAtomicLocaleReloadRollover(
    context: AdmissionContext,
    targetLocale: AdmissionLocale,
    reload: () => Promise<unknown>,
    awaitReloadReadiness: AdmissionReloadReadiness,
  ) {
    const setupWindow = this.activeCaptureWindow;
    if (!setupWindow || !sameAdmissionContext(setupWindow.context, context))
      throw new Error(
        `Cannot rollover a non-current capture window: ${context.matrix}/${context.scenario}/${context.route}`,
      );
    if (setupWindow.cause !== 'initial-navigation')
      throw new Error(
        `Atomic rollover requires an initial-navigation setup window, found ${setupWindow.cause}.`,
      );
    if (this.replacementCaptureWindow) throw new Error('Atomic rollover is already active.');
    if (this.declaredPendingRequests.has(setupWindow.captureWindowId))
      throw new Error(
        `Cannot rollover a capture window with a declared pending request: ${setupWindow.captureWindowId}`,
      );
    const source = await this.observeLocaleReloadSource();
    const replacement: AdmissionCaptureWindow = {
      captureWindowId: this.nextCaptureWindowId++,
      navigationEpoch: this.navigationEpoch + 1,
      context,
      sourceLocale: source.sourceLocale,
    ...
    this.replacementCaptureWindow = replacement;
    this.navigationEpoch = replacement.navigationEpoch;
    let reloadCommitted = false;
    try {
      await reload();
      reloadCommitted = true;
      await this.waitForWindowTerminal(setupWindow, 'setup');
      this.activeCaptureWindow = replacement;
      this.replacementCaptureWindow = undefined;
      await awaitReloadReadiness();
      await this.waitForWindowTerminal(replacement, 'reload');
      const setupLifecycles = this.requestLifecycles.filter(
        (lifecycle) => lifecycle.captureWindowId === setupWindow.captureWindowId,
      );
      const replacementLifecycles = this.requestLifecycles.filter(
        (lifecycle) => lifecycle.captureWindowId === replacement.captureWindowId,
      );
      const evidence = this.createRetiredSetupLifecycleEvidence(
        setupWindow,
        replacement,
        setupLifecycles,
        replacementLifecycles,
      );
      this.retiredSetupEvidence.set(replacement.captureWindowId, evidence);
      this.requestFailures.delete(setupWindow.captureWindowId);
    } catch (error) {
      if (reloadCommitted) this.activeCaptureWindow = replacement;
      this.replacementCaptureWindow = undefined;
      throw error;
    }
  }

  private async waitForWindowTerminal(window: AdmissionCaptureWindow, role: 'setup' | 'reload') {
    for (let attempt = 0; attempt < 25; attempt += 1) {
      const unresolved = [...this.pendingRequests.values()].filter(
        (request) => request.captureWindow.captureWindowId === window.captureWindowId,
      );
    ...
    throw new Error(
      `Cannot rollover capture window with unresolved ${role} request(s): ${unresolved.map((request) => request.requestId).join(',')}`,
    );
  }

  private createRetiredSetupLifecycleEvidence(
    setupWindow: AdmissionCaptureWindow,
    reloadWindow: AdmissionCaptureWindow,
    setupLifecycles: readonly AdmissionRequestLifecycle[],
    reloadLifecycles: readonly AdmissionRequestLifecycle[],
  ): AdmissionRetiredSetupLifecycleEvidence {
    if (
      !setupLifecycles.length ||
      new Set(setupLifecycles.map((lifecycle) => lifecycle.requestId)).size !==
        setupLifecycles.length
    )
      throw new Error(
        `Cannot rollover capture window with missing or duplicate setup lifecycle identity: ${setupWindow.captureWindowId}`,
      );
    const replacementIds = new Set<number>();
    const replacementPairs: AdmissionRetiredSetupReplacementPair[] = [];
    for (const setup of setupLifecycles) {
      if (setup.method !== 'GET')
        throw new Error(`Cannot rollover capture window with setup write: ${setup.requestId}`);
      if (
        setup.outcome === 'response' &&
        setup.status !== undefined &&
        setup.status >= 200 &&
        setup.status < 300
      )
        continue;
      if (setup.outcome !== 'failure' || setup.error !== 'net::ERR_ABORTED')
        throw new Error(
          `Cannot rollover capture window with blocking setup terminal outcome: ${setup.requestId}`,
        );
      const replacement = reloadLifecycles.find(
        (candidate) =>
          !replacementIds.has(candidate.requestId) &&
          candidate.requestId !== setup.requestId &&
          candidate.sequence > setup.sequence &&
          candidate.method === 'GET' &&
          candidate.path === setup.path &&
          candidate.outcome === 'response' &&
          typeof candidate.status === 'number' &&
          candidate.status >= 200 &&
          candidate.status < 300 &&
          candidate.captureWindowId === reloadWindow.captureWindowId &&
          candidate.navigationEpoch === reloadWindow.navigationEpoch &&
          sameAdmissionContext(candidate.context, setupWindow.context) &&
          candidate.sourceLocale === setupWindow.targetLocale &&
          candidate.targetLocale === reloadWindow.targetLocale &&
          candidate.cause === 'locale-reload' &&
          candidate.phase === reloadWindow.phase,
      );
      if (!replacement || replacement.status === undefined)
        throw new Error(
          `Cannot rollover setup abort without a later exact reload replacement: ${setup.requestId}`,
        );
      replacementIds.add(replacement.requestId);
      replacementPairs.push({
        setupRequestId: setup.requestId,
        replacementRequestId: replacement.requestId,
        setupSequence: setup.sequence,
    ...
    return {
      ...
      setupLifecycles: [...setupLifecycles],
      replacementLifecycles: [...reloadLifecycles],
      replacementPairs,
    };
  }

  declarePendingRequest(
    context: AdmissionContext,
    targetLocale: AdmissionLocale,
    declaration: AdmissionPendingRequestDeclaration,
  ) {
    const window = this.activeCaptureWindow;
    if (
      !window ||
      !sameAdmissionContext(window.context, context) ||
      window.targetLocale !== targetLocale
    )
      throw new Error(
        `Cannot declare a pending request outside the exact capture window: ${context.matrix}/${context.scenario}/${context.route}/${targetLocale}`,
      );
    if (!declaration.method || !declaration.path.startsWith('/'))
      throw new Error('Declared pending request requires a method and absolute path.');
    if (this.declaredPendingRequests.has(window.captureWindowId))
      throw new Error(
        `Capture window already has a declared pending request: ${window.captureWindowId}`,
      );
    if (
      ...
      throw new Error(
        `Cannot declare a pending request while another request remains open in capture window: ${window.captureWindowId}`,
      );
    this.declaredPendingRequests.set(window.captureWindowId, {
      declaration,
  ...
  endCaptureWindow(context: AdmissionContext, targetLocale: AdmissionLocale) {
    ...
    if (
      ...
      throw new Error(
        `Cannot close a non-current capture window: ${context.matrix}/${context.scenario}/${context.route}/${targetLocale}`,
      );
    const unresolved = [...this.pendingRequests.values()].filter(
      (request) => request.captureWindow.captureWindowId === window.captureWindowId,
    );
    ...
    if (declaration) {
      if (!declaration.request || unresolved.length !== 1 || unresolved[0] !== declaration.request)
        throw new Error(
          `Cannot close capture window without its one declared pending request: ${unresolved.map((request) => request.requestId).join(',')}`,
        );
      declaration.closed = true;
    } else if (unresolved.length)
      throw new Error(
        `Cannot close capture window with unresolved request(s): ${unresolved.map((request) => request.requestId).join(',')}`,
      );
    this.retiredSetupEvidence.delete(window.captureWindowId);
    this.activeCaptureWindow = undefined;
  }

  private async awaitCaptureWindowSettled(
    context: AdmissionContext,
    targetLocale: AdmissionLocale,
  ) {
    const window = this.activeCaptureWindow;
    if (
      ...
      throw new Error(
        `Cannot settle a non-current capture window: ${context.matrix}/${context.scenario}/${context.route}/${targetLocale}`,
      );
    const declaration = this.declaredPendingRequests.get(window.captureWindowId);
    for (let attempt = 0; attempt < 25; attempt += 1) {
      const unresolved = [...this.pendingRequests.values()].filter(
      ...
      if (declaration) {
        if (declaration.request && unresolved.length === 1 && unresolved[0] === declaration.request)
          return;
        if (declaration.request && !unresolved.length)
          throw new Error(
            `Declared pending request became terminal before evidence capture: ${declaration.request.requestId}`,
          );
        if (
          unresolved.length &&
          (!declaration.request || unresolved.some((request) => request !== declaration.request))
        )
          throw new Error(
            `Capture window has undeclared or extra pending request(s): ${unresolved.map((request) => request.requestId).join(',')}`,
          );
    ...
    throw new Error(
      declaration
        ? `Declared pending request did not start before evidence capture: ${window.captureWindowId}`
        : `Capture window did not settle before evidence capture: ${unresolved.map((request) => request.requestId).join(',')}`,
    );
  }

  private supersededReadLifecycles(
    lifecycles: readonly AdmissionRequestLifecycle[],
  ): AdmissionSupersededReadLifecycle[] {
    ...
    for (const failed of lifecycles) {
      ...
      const replacement = lifecycles.find(
        (candidate) =>
          ...
          candidate.sourceLocale === failed.sourceLocale &&
          candidate.targetLocale === failed.targetLocale &&
          candidate.cause === failed.cause &&
          candidate.phase === failed.phase &&
          candidate.outcome === 'response' &&
          typeof candidate.status === 'number' &&
          candidate.status >= 200 &&
          candidate.status < 300,
      ...
      pairs.push({
        failedRequestId: failed.requestId,
        replacementRequestId: replacement.requestId,
        failedSequence: failed.sequence,
        replacementSequence: replacement.sequence,
        method: 'GET',
        path: failed.path,
        captureWindowId: failed.captureWindowId,
        navigationEpoch: failed.navigationEpoch,
        context: failed.context,
        sourceLocale: failed.sourceLocale,
        targetLocale: failed.targetLocale,
        cause: failed.cause,
        phase: failed.phase,
        replacementStatus,
        targetRendered: true,
  ...
  private diagnosticsSinceLastCapture(context: AdmissionContext, targetLocale: AdmissionLocale) {
    ...
    if (
      ...
      throw new Error(
        `Capture context has no exact request-lifecycle window: ${context.matrix}/${context.scenario}/${context.route}/${targetLocale}`,
      );
    const unresolved = [...this.pendingRequests.values()].filter(
      (request) => request.captureWindow.captureWindowId === window.captureWindowId,
    );
    ...
    if (
      declaration &&
      (!declaration.request || unresolved.length !== 1 || unresolved[0] !== declaration.request)
    )
      throw new Error(
        `Capture window cannot serialize an invalid declared pending request: ${unresolved.map((request) => request.requestId).join(',')}`,
      );
    if (!declaration && unresolved.length)
      throw new Error(
        `Capture window has unresolved request(s): ${unresolved.map((request) => request.requestId).join(',')}`,
      );
    if (
      this.navigationTeardowns.length &&
      (context.matrix !== 'M01' ||
        context.scenario !== 'completion-ready' ||
        context.route !== '/learning/enrollments/4')
    )
      throw new Error(
        `Navigation teardown cannot bind to this capture context: ${context.matrix}/${context.scenario}/${context.route}`,
      );
    ...
    const requestLifecycles = this.requestLifecycles.filter(
      (lifecycle) => lifecycle.captureWindowId === window.captureWindowId,
    );
    const retiredSetupLifecycleEvidence = this.retiredSetupEvidence.get(window.captureWindowId);
    ...
    const requestFailures = (this.requestFailures.get(window.captureWindowId) ?? []).filter(
      (failure) => {
    ...
    const diagnostics = {
      ...
      requestFailures,
      requestLifecycles,
      retiredSetupLifecycleEvidence: retiredSetupLifecycleEvidence ?? null,
      supersededReadLifecycles,
      declaredPendingRequests: declaration
        ? [
            {
              requestId: declaration.request!.requestId,
              method: declaration.request!.method,
              path: declaration.request!.path,
              captureWindowId: window.captureWindowId,
              navigationEpoch: window.navigationEpoch,
              context: window.context,
              sourceLocale: window.sourceLocale,
              ...
              cause: window.cause,
              phase: window.phase,
              outcome: 'declared_pending' satisfies AdmissionDeclaredPendingRequest['outcome'],
            },
          ]
        : [],
    ...
    this.requestLifecycles.splice(
      0,
      this.requestLifecycles.length,
      ...this.requestLifecycles.filter(
        (lifecycle) => lifecycle.captureWindowId !== window.captureWindowId,
      ),
    );
  ...
  async finalizeDeclaredPendingRequest(
    context: AdmissionContext,
    targetLocale: AdmissionLocale,
    expected: Pick<AdmissionRequestLifecycle, 'outcome' | 'status' | 'error'>,
  ) {
    const declaration = [...this.declaredPendingRequests.values()].find(
      (candidate) =>
        sameAdmissionContext(candidate.captureWindow.context, context) &&
        candidate.captureWindow.targetLocale === targetLocale,
    );
    if (!declaration || !declaration.closed || !declaration.request)
      throw new Error('Declared pending request is not retained from a closed capture window.');
    for (let attempt = 0; attempt < 25; attempt += 1) {
      const lifecycle = this.requestLifecycles.filter(
        (candidate) => candidate.requestId === declaration.request!.requestId,
      );
      if (lifecycle.length === 1) {
        const terminal = lifecycle[0];
        if (
          terminal.outcome !== expected.outcome ||
          terminal.status !== expected.status ||
          terminal.error !== expected.error
        )
          throw new Error(
            `Declared pending request reached an unexpected terminal outcome: ${terminal.requestId}`,
          );
        const pending = {
          requestId: declaration.request.requestId,
          method: declaration.request.method,
          path: declaration.request.path,
          captureWindowId: declaration.captureWindow.captureWindowId,
          navigationEpoch: declaration.captureWindow.navigationEpoch,
          context: declaration.captureWindow.context,
          sourceLocale: declaration.captureWindow.sourceLocale,
          targetLocale: declaration.captureWindow.targetLocale,
          cause: declaration.captureWindow.cause,
          phase: declaration.captureWindow.phase,
          outcome: 'declared_pending' satisfies AdmissionDeclaredPendingRequest['outcome'],
        };
        const release = {
        ...
        await writeFile(
          this.testInfo.outputPath('declared-pending-release.json'),
      ...
      if (lifecycle.length > 1)
        throw new Error(
          `Declared pending request has duplicate terminal outcomes: ${declaration.request.requestId}`,
        );
      await this.page.waitForTimeout(20);
    }
    throw new Error(
      `Declared pending request did not reach its terminal outcome: ${declaration.request.requestId}`,
    );
  }
  ...
  async observedLocaleSnapshot(): Promise<ObservedLocaleConvergence> {
    const rawLocale = await this.readRawLocaleObservation().catch(() => ({
      documentLocale: '',
      storedLocale: null,
    }));
  ...
  async observedLocaleConvergence(): Promise<StrictLocaleConvergence> {
    ...
    if (
      ...
      !locale.storagePresent ||
      !locale.storageLocale ||
      locale.storageLocale !== locale.documentLocale
    )
      throw new Error(
        `Unavailable or mismatched observed locale: document=${rawLocale.documentLocale || 'missing'} storage=${rawLocale.storedLocale ?? 'missing'}`,
      );
    return {
      documentLocale: locale.documentLocale,
      storageLocale: locale.storageLocale,
      storagePresent: true,
    };
  }
  ...
  async observeLocaleReloadSource(): Promise<LocaleReloadSource> {
    ...
    if (
      this.navigationEpochs.currentEpoch() === 0 &&
      rawLocale.documentLocale === 'en' &&
      rawLocale.storedLocale === null
    )
      return {
        sourceLocale: 'en',
        bootstrap: {
    ...
    throw new Error(
      `Unavailable or mismatched locale-reload source: document=${rawLocale.documentLocale || 'missing'} storage=${rawLocale.storedLocale ?? 'missing'}`,
    );
  }
  ...
  async prepareForNavigation(
    ...
    this.navigationEpochs.begin(
      cause,
      source ?? { sourceLocale: await this.observedLocale() },
      allowedPaths,
    );
    for (const request of this.pendingRequests.keys()) this.navigationEpochs.addExisting(request);
  }

  async prepareCatalogHeroNavigation(
    cause: 'locale-reload' | 'capture-route-navigation',
    targetLocale: AdmissionLocale,
  ) {
    if (this.catalogHeroBoundary)
      throw new Error('Catalog hero lifecycle boundary was not finalized before a new navigation.');
    this.catalogHeroBoundary = {
      epoch: this.catalogHeroEpoch + 1,
      cause,
      sourceLocale: await this.observedLocale(),
      targetLocale,
      phase: 'pre_document_commit',
  ...
  async confirmCatalogHeroRendered() {
    const boundary = this.catalogHeroBoundary;
    if (!boundary || !boundary.documentCommitted || boundary.phase !== 'post_document_commit')
      throw new Error('Catalog hero render confirmation requires a committed lifecycle boundary.');
    if ((await this.observedLocale()) !== boundary.targetLocale)
      throw new Error('Catalog hero render locale does not match the lifecycle target locale.');
      ...
      (path) =>
        [...document.querySelectorAll<HTMLElement>('*')].some((element) => {
          ...
          return (
            box.width > 0 &&
            box.height > 0 &&
            getComputedStyle(element, '::before').backgroundImage.includes(path)
          );
        }),
      catalogHeroAssetPath,
    );
    if (!rendered)
      throw new Error('Catalog hero asset was not rendered after its navigation boundary.');
    for (let index = 0; index < this.catalogHeroLifecycles.length; index += 1) {
      const event = this.catalogHeroLifecycles[index];
      if (event.kind === 'replacement' && event.boundary.epoch === boundary.epoch)
        this.catalogHeroLifecycles[index] = { ...event, catalogHeroRendered: true };
    }
  ...
  private async interactionLocator() {
    if (
      ...
      (await this.interactionTarget.isVisible().catch(() => false)) &&
      (await this.interactionTarget.isEnabled().catch(() => false))
    )
      return this.interactionTarget;
    const candidates = this.page.locator(
      'button:not([disabled]):visible, a[href]:not([href^="#"]):visible, input:not([disabled]):visible, select:not([disabled]):visible, textarea:not([disabled]):visible, [role="button"]:not([aria-disabled="true"]):visible, [role="dialog"]:visible',
    );
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      if (
        await candidate.evaluate((element) => document.activeElement === element).catch(() => false)
      )
        return candidate;
    }
  ...
  private async measureInteraction(
    context: AdmissionContext,
  ): Promise<AdmissionInteractionEvidence> {
    ...
    if (
      context.matrix === 'M08' &&
      context.scenario === 'clear-pending' &&
      context.state === 'clear-pending' &&
      context.session === 'authenticated'
    ) {
      const dialog = this.page.getByRole('dialog');
      const targetName =
        (await dialog.getAttribute('aria-label')) ??
        (await dialog.textContent())?.trim() ??
        'unnamed-modal-dialog';
      ...
      for (let index = 0; index < actionCount; index += 1) {
        ...
        if (
          ...
          enabledDialogActions += 1;
      }
      await dialog.focus().catch(() => undefined);
      const initialDialogFocus = await dialog
        .evaluate((element) => document.activeElement === element)
        .catch(() => false);
      await this.page.keyboard.press('Tab');
      const tabFocusWithinDialog = await dialog
        .evaluate((element) => element.contains(document.activeElement))
        .catch(() => false);
      const noBackgroundFocus = tabFocusWithinDialog;
      const focusPass = visible && initialDialogFocus;
      const keyboardPass =
        focusPass && tabFocusWithinDialog && noBackgroundFocus && enabledDialogActions === 0;
      const geometryPass = Boolean(box && box.width >= 44 && box.height >= 44);
      return {
        mode: 'modal_focus_containment',
        target: targetName,
        focus: {
          status: focusPass ? 'pass' : 'fail',
          ...(focusPass
            ? {}
            : { reason: 'Pending clear dialog did not receive initial browser focus.' }),
        },
        keyboard: {
          ...
          ...(keyboardPass
            ? {}
            : {
                reason: `Pending clear dialog containment failed: within=${tabFocusWithinDialog}, background=${noBackgroundFocus}, enabledActions=${enabledDialogActions}.`,
              }),
        },
        minTarget44: {
          status: geometryPass ? 'pass' : 'fail',
          ...(geometryPass
            ? {}
            : {
                reason: `Dialog geometry was ${box ? `${box.width}x${box.height}` : 'unavailable'} CSS pixels; 44x44 is required.`,
              }),
        },
        modal: {
          initialDialogFocus,
          tabFocusWithinDialog,
          noBackgroundFocus,
          enabledDialogActions,
        },
      };
    }
    ...
    return {
      ...
      target: targetName,
      focus: {
        status: focus ? 'pass' : 'fail',
        ...(focus ? {} : { reason: 'Named target did not receive browser focus.' }),
      },
      keyboard: {
        status: keyboard ? 'pass' : 'fail',
        ...(keyboard ? {} : { reason: 'Tab did not move browser focus from the named target.' }),
      },
      minTarget44: {
        ...
        ...(box && box.width >= 44 && box.height >= 44
          ? {}
          : {
              reason: `Target geometry was ${box ? `${box.width}x${box.height}` : 'unavailable'} CSS pixels; 44x44 is required.`,
            }),
      },
    };
  }
  ...
  async capture(
    ...
    if (observedLocale !== locale)
      throw new Error(
        `Capture locale does not match observed document locale: expected=${locale} observed=${observedLocale}`,
      );
    const cellId = [context.matrix, context.scenario, locale, width, effectiveZoom]
    ...
    await cdp.send('Emulation.setPageScaleFactor', {
      pageScaleFactor: effectiveZoom === 200 ? 2 : 1,
    });
    await cdp.detach();
    const captureScreenshot =
      Boolean(selectedAdmissionCell) ||
      screenshotMode === 'full' ||
      (effectiveZoom === 100 &&
        canonicalScreenshotWidths.includes(width as (typeof canonicalScreenshotWidths)[number]));
    ...
    if (captureScreenshot) {
      ...
      screenshot = {
        kind: 'captured',
        path: join('screenshots', basename(screenshotPath)),
        sha256: await fileHash(screenshotPath),
      };
    ...
    const record = {
      ...
      interaction,
      diagnostics: this.diagnosticsSinceLastCapture(context, observedLocale),
  ...
  async recordRoutedObservation(
    undo: Locator,
    complete: Locator,
    observation: RoutedProductObservation,
  ) {
    ...
    const screenshotPath = this.testInfo.outputPath(
      'screenshots',
      'M01--completion-ready--undo-space-routed-observation.png',
    );
    await mkdir(join(this.testInfo.outputDir, 'screenshots'), { recursive: true });
    await this.page.screenshot({ path: screenshotPath, fullPage: true });
    const record = {
      ...
      diagnostics: this.diagnosticsSinceLastCapture(
        {
          matrix: 'M01',
          scenario: 'completion-ready',
          route: '/learning/enrollments/4',
    ...
    await writeFile(
      this.testInfo.outputPath('routed-observation.json'),
  ...
  async recordM01Outcome(undo: Locator, complete: Locator) {
    ...
    const screenshotPath = this.testInfo.outputPath(
      'screenshots',
      'M01--completion-ready--undo-space-current-outcome.png',
    );
    await mkdir(join(this.testInfo.outputDir, 'screenshots'), { recursive: true });
    await this.page.screenshot({ path: screenshotPath, fullPage: true });
    const record = {
    ...
    await writeFile(
      this.testInfo.outputPath('m01-outcome.json'),
      `${JSON.stringify(record, null, 2)}\n`,
      { encoding: 'utf8', flag: 'wx' },
    );
    this.endCaptureWindow(
...
export async function selectLocale(
  ...
  page: Page,
  locale: AdmissionLocale,
  allowedTeardownPaths: readonly string[] = [],
  catalogHeroLifecycle = false,
) {
...
export async function captureDeclaredMatrix(
  recorder: AdmissionRecorder,
  context: AdmissionContext,
  locale: AdmissionLocale,
) {
  ...
  for (const capture of captures) {
    const cellId = [context.matrix, context.scenario, locale, capture.width, capture.effectiveZoom]
      .map((value) => segment(String(value)))
      .join('--');
    if (!selectedCell || selectedCell === cellId)
      await recorder.capture(context, locale, capture.width, capture.effectiveZoom);
  }
