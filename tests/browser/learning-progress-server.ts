...
export default async function startLearningProgressServer() {
  ...
  if (server.config.define?.['import.meta.env.VITE_API_BASE_URL'] !== learningProgressApiDefinition) {
    await server.close();
    throw new Error('Learning-progress Vite API origin does not match the deterministic route harness');
  }
