...
import {
  CATALOG_SORT_VALUES, parseCatalogQuery, serializeCatalogQuery, toCourseListQuery, validateCatalogDraft,
...
describe('catalog URL query contract', () => {
  ...
  it('normalizes invalid URL values and blocks invalid range submission', () => {
    expect(parseCatalogQuery(new URLSearchParams('q=nope&minPrice=3&page=0&sort=rating')))
      .toEqual({ sort: 'created_at', page: 1, search_query: undefined, min_price: undefined, max_price: undefined });
    expect(validateCatalogDraft({ search_query: 'React', min_price: '30', max_price: '10', sort: 'created_at' }))
  ...
  it('uses created_at for the clean Oldest default while canonicalizing legacy ID bookmarks', () => {
    expect(CATALOG_SORT_VALUES).toEqual(['created_at', '-created_at', 'price', '-price', 'title', '-title']);
    ...
    expect(toCourseListQuery(defaultQuery)).toMatchObject({ sort: 'created_at', page: 1, page_size: 20 });
