...
const validResponse = {
  ...
  page: 2, page_size: 20, total: 21, pages: 2, has_next: false, has_previous: true,
};
...
describe('catalog request boundary', () => {
  it('uses the exact API-008 request object with the fixed page size', async () => {
    ...
    expect(request).toHaveBeenCalledWith(expect.objectContaining({
      method: 'GET', path: '/courses', query: { search_query: 'React', min_price: 5, max_price: 10, sort: '-price', page: 2, page_size: 20 },
    }));
  });
