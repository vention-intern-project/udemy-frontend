...
import { learningListQueryKey } from '../../src/features/learning-progress';
...
const enrollments = {
  items: [
    { id: 4, user_id: 1, course_id: 7, status: 'cancelled', created_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z', course: { id: 7, title: 'Cancelled course', description: null, price: '10.00', currency: 'USD' } },
    { id: 5, user_id: 1, course_id: 8, status: 'active', created_at: '2026-01-02T00:00:00Z', updated_at: '2026-01-02T00:00:00Z', course: { id: 8, title: 'Active course', description: 'Server order must remain intact.', price: '0.00', currency: 'USD' } },
  ], page: 2, page_size: 20, total: 22, pages: 2, has_next: false, has_previous: true,
};
...
function LocationProbe() {
  ...
  return <>
    ...
    <button type="button" onClick={() => navigate('/learning?page=2')}>Navigate to page 2</button>
  </>;
}
...
async function renderPage(request: ApiClient['request'], initialEntry = '/learning?page=2') {
  ...
  await act(async () => {
    render(<QueryClientProvider client={queryClient}><SessionProvider client={{ request }} tokenStore={tokenStore()}><MemoryRouter initialEntries={[initialEntry]}><LocationProbe /><LearningListPage /></MemoryRouter></SessionProvider></QueryClientProvider>);
  });
  return queryClient;
}
...
async function renderApp(request: ApiClient['request']) {
  await act(async () => {
    render(<QueryClientProvider client={createAppQueryClient()}><SessionProvider client={{ request }} tokenStore={tokenStore()}><ThemeProvider initialDensityMode="workspace"><MemoryRouter initialEntries={['/learning']}><AppRouter /></MemoryRouter></ThemeProvider></SessionProvider></QueryClientProvider>);
  });
}
...
describe('LearningListPage', () => {
  it('preserves API-021 server order, totals, status, and page cursor', async () => {
    ...
    expect(screen.getAllByRole('heading', { level: 2 }).map((node) => node.textContent)).toEqual(['Cancelled course', 'Active course']);
    expect(screen.getByText('Cancelled')).toBeTruthy();
    expect(requests.find((entry) => entry.path === '/enrollments/my')?.query).toEqual({ page: 2, page_size: 20 });
  });
  ...
  it('retries a list error and restores focus to the recovered heading', async () => {
    ...
    const request = vi.fn(async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, student);
      if (options.path === '/enrollments/my' && !failedOnce) { failedOnce = true; throw new ApiError({ kind: 'server', status: 500, message: 'private' }); }
      if (options.path === '/enrollments/my') return decode(options, { ...enrollments, page: 1, pages: 2, has_next: true, has_previous: false });
      throw new Error(`Unexpected request ${options.path}`);
    });
  ...
  it('restores focus across uncached and cached API-021 pages without stealing it on background refetch', async () => {
    ...
    const backgroundRefresh = new Promise<void>((resolve) => { resolveBackgroundRefresh = resolve; });
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, student);
      if (options.path === '/enrollments/my') {
        ...
        const items = page === 1 ? [
          { ...enrollments.items[1], id: 6, course_id: 9, course: { ...enrollments.items[1].course, id: 9, title: 'First page course' } },
        ] : enrollments.items;
        return decode(options, { ...enrollments, items, page, has_next: page === 1, has_previous: page === 2 });
      }
      throw new Error(`Unexpected request ${options.path}`);
    };
    ...
    const user = userEvent.setup();
    await act(async () => { await user.click(screen.getByRole('button', { name: 'Go to next page' })); });
    ...
    expect(screen.getAllByRole('heading', { level: 2 }).map((node) => node.textContent)).toEqual(['Cancelled course', 'Active course']);
    await waitFor(() => expect(document.activeElement).toBe(screen.getByRole('heading', { name: 'My learning' })));

    await act(async () => { await user.click(screen.getByRole('button', { name: 'Go to previous page' })); });
    expect(await screen.findByText('22 enrollments · Page 1 of 2')).toBeTruthy();
    expect(screen.getByLabelText('Current location').textContent).toBe('');
    expect(screen.getAllByRole('heading', { level: 2 }).map((node) => node.textContent)).toEqual(['First page course']);
    await waitFor(() => expect(document.activeElement).toBe(screen.getByRole('heading', { name: 'My learning' })));
    ...
    await act(async () => { refresh = queryClient.refetchQueries({ queryKey: learningListQueryKey(student.email, 1), exact: true }); });
    await waitFor(() => expect(requestedPages).toEqual([1, 2, 1, 1]));
    await act(async () => { resolveBackgroundRefresh?.(); await refresh; });
  ...
  it('cancels interrupted page focus intent before later history-independent page success', async () => {
    ...
    const pageTwoResponse = new Promise<void>((resolve) => { resolvePageTwo = resolve; });
    const requestedPages: number[] = [];
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, student);
      if (options.path === '/enrollments/my') {
        const page = Number(options.query?.page);
        requestedPages.push(page);
        if (page === 2 && requestedPages.filter((requestedPage) => requestedPage === 2).length === 1) await pageTwoResponse;
        const items = page === 1 ? [
          { ...enrollments.items[1], id: 6, course_id: 9, course: { ...enrollments.items[1].course, id: 9, title: 'First page course' } },
        ] : enrollments.items;
        return decode(options, { ...enrollments, items, page, has_next: page === 1, has_previous: page === 2 });
    ...
    await screen.findByText('22 enrollments · Page 1 of 2');
    await act(async () => { await user.click(screen.getByRole('button', { name: 'Go to next page' })); });
    await waitFor(() => expect(screen.getByLabelText('Current location').textContent).toBe('?page=2'));
    await waitFor(() => expect(requestedPages).toEqual([1, 2]));

    const historyBack = screen.getByRole('button', { name: 'History back' });
    await act(async () => { await user.click(historyBack); });
    await screen.findByText('22 enrollments · Page 1 of 2');
    await waitFor(() => expect(document.activeElement).toBe(historyBack));
    await act(async () => { resolvePageTwo?.(); await Promise.resolve(); });
    ...
    await act(async () => { await user.click(nonPaginationNavigation); });
  ...
  it('keeps a malformed API-021 success distinct from the empty state', async () => {
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, student);
      if (options.path === '/enrollments/my') throw new ApiError({ kind: 'invalid_response', status: 200, message: 'private decoder detail' });
      throw new Error(`Unexpected request ${options.path}`);
    };
    await renderPage(request, '/learning');
    expect(await screen.findByText('The server returned an invalid response. Try again.')).toBeTruthy();
    expect(screen.queryByRole('heading', { name: 'Your learning is empty' })).toBeNull();
  ...
  it('clears a list 401 and routes the protected workspace to login', async () => {
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, student);
      if (options.path === '/enrollments/my') throw new ApiError({ kind: 'unauthorized', status: 401, message: 'private auth detail' });
      throw new Error(`Unexpected request ${options.path}`);
    };
