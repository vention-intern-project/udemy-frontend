...
function RouteLayoutRaceProbe({
  ...
  return <button type="button" onClick={() => navigate(destination)}>Commit route transition</button>;
}
...
describe('authentication pages', () => {
  ...
  it.each([
  ...
  ])('keeps every password reveal independent and accessible on $route', async ({ route, passwordIds }) => {
    ...
    for (const passwordId of passwordIds) {
      ...
      const button = input.parentElement?.querySelector<HTMLButtonElement>('button') as HTMLButtonElement;
      button.focus();
    ...
    expect(document.getElementById(email ? 'email' : passwordIds[0])).toBe(document.activeElement);
    for (const passwordId of passwordIds) {
      const input = document.getElementById(passwordId) as HTMLInputElement;
      const button = input.parentElement?.querySelector<HTMLButtonElement>('button');
  ...
  it('maps hostile 422 issues to stable field copy without rendering backend messages', async () => {
    const hostileMarkers = ['PRIVATE_EMAIL_DIAGNOSTIC', 'PRIVATE_PASSWORD_DIAGNOSTIC', 'PRIVATE_UNKNOWN_FIELD'];
    renderAuth('/login', async () => {
      throw new ApiError({
        ...
        issues: [
          ...
          { location: ['body', 'password'], message: hostileMarkers[1], type: 'vendor_private_rule' },
          { location: ['body', 'internal'], message: hostileMarkers[2], type: 'vendor_private_rule' },
        ],
      });
    });
    const user = userEvent.setup();
    ...
    expect(document.getElementById('password-error')?.textContent).toBe('Check this field and submit again.');
  ...
  it.each([
    ...
    ['reset', '/reset-password?token=private-reset-token', '/reset-password', 'Reset password', 'Resetting password...'],
  ] as const)('synchronously excludes duplicate %s submissions', async (
    workflow,
    pagePath,
    operationPath,
    idleButtonName,
    ...
    expect(screen.getByRole('button', { name: pendingButtonName }).hasAttribute('disabled')).toBe(true);
    if (workflow !== 'forgot') {
    ...
    await waitFor(() => expect(
      screen.getByRole('button', { name: idleButtonName }).hasAttribute('disabled'),
    ).toBe(false));
  ...
  it.each([
    ...
    ['forgot', 'resolve', '/forgot-password', '/forgot-password?race=destination', '/forgot-password', 'Continue', 'Submitting request...'],
    ['forgot', 'reject', '/forgot-password', '/forgot-password?race=destination', '/forgot-password', 'Continue', 'Submitting request...'],
    ['reset', 'resolve', '/reset-password?token=private-reset-token', '/reset-password?token=private-reset-token&race=destination', '/reset-password', 'Reset password', 'Resetting password...'],
    ['reset', 'reject', '/reset-password?token=private-reset-token', '/reset-password?token=private-reset-token&race=destination', '/reset-password', 'Reset password', 'Resetting password...'],
  ] as const)(
    ...
    async (
      ...
      await interact(() => user.click(screen.getByRole('button', { name: 'Commit route transition' })));
  ...
  it('maps duplicate signup safely, retries explicitly, and strips confirmation from the DTO', async () => {
    ...
    const { request, store } = renderAuth('/signup', async (options) => {
      ...
      signupAttempts += 1;
      if (signupAttempts === 1) {
        throw new ApiError({ kind: 'bad_request', status: 400, message: 'private duplicate detail' });
      }
      return { access_token: 'signup-token', token_type: 'bearer', user: { id: 1, email: 'learner@example.com' } };
    });
    const user = userEvent.setup();
    await screen.findByLabelText(/^Email/);
    await interact(() => user.type(screen.getByLabelText(/^Email/), 'learner@example.com'));
    ...
    await interact(() => user.type(screen.getByLabelText(/^Confirm password/), 'plain required value'));
    await interact(() => user.click(screen.getByRole('button', { name: 'Create account' })));
    ...
    expect(signupOptions?.body).toEqual({
      email: 'learner@example.com', name: 'Ada', surname: 'Lovelace',
      password: 'plain required value', role: 'student',
    });
  ...
  it.each([
    ...
    ['signup', '/signup', '/signup', 'Create account', { access_token: 'token', token_type: 'bearer' }],
  ...
  it('shows neutral forgot-password success without account or delivery disclosure', async () => {
    const { request } = renderAuth('/forgot-password', async () => ({ message: 'private server detail' }));
    const user = userEvent.setup();
    await screen.findByLabelText(/^Email/);
    ...
    expect(status?.textContent).not.toMatch(/email (was )?sent|delivery succeeded|account exists|private server detail/i);
    expect(request).toHaveBeenCalledWith(expect.objectContaining({
      path: '/forgot-password', authPolicy: 'public', body: { email: 'person@example.com' },
    }));
  });
  ...
  it('redirects a missing reset token without rendering a token field', async () => {
    ...
    expect(screen.getByLabelText('location').textContent).toBe('/forgot-password?reason=missing-token');
  ...
  it('maps invalid reset tokens to a stable public error and allows safe retry', async () => {
    ...
    const { request } = renderAuth('/reset-password?token=private-reset-token', async () => {
      attempts += 1;
      if (attempts === 1) throw new ApiError({ kind: 'bad_request', status: 400, message: 'raw token detail' });
      return { message: 'ok' };
    });
    const user = userEvent.setup();
