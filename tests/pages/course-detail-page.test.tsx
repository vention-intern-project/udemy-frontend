...
const enrollmentMutation = {
  ...
  course: { id: 7, title: course.title, description: course.description, price: '0.00', currency: course.currency },
};
...
function outline(downloadUrl: string | null) {
  return {
    items: [lesson(downloadUrl)], page: 1, page_size: 100, total: 1, pages: 1,
...
function PageHarnessControls({ sessionControls, routeControls }: PageHarnessOptions) {
  ...
  return (
    <>
      {routeControls ? <button type="button" onClick={() => navigate('/courses/8')}>Open course 8</button> : null}
      {routeControls ? <button type="button" onClick={() => navigate('/courses/7')}>Return to course 7</button> : null}
      {sessionControls ? <button type="button" onClick={session.clearSession}>Clear session</button> : null}
    </>
  );
}
...
describe('CourseDetailPage', () => {
  it.each([
    ...
    expect(await screen.findByRole('heading', { level: 1, name: 'React foundations' })).toBeTruthy();
    ...
    expect(screen.getByRole('link', { name: 'Log in to enroll free' }).getAttribute('href'))
      .toBe('/login?returnTo=%2Fcourses%2F7');
  ...
  it('clears a genuine invalid bearer after 401 before rendering public redacted metadata', async () => {
    ...
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      ...
      if (options.path === '/me') throw new ApiError({ kind: 'unauthorized', status: 401, message: 'Could not validate credentials' });
      if (options.path === '/courses/7') return decode(options, course);
  ...
  it('retries only the failed outline query and exposes an empty outline distinctly', async () => {
    ...
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      ...
      if (options.path === '/courses/7/lessons') {
        outlineAttempts += 1;
        if (outlineAttempts === 1) throw new ApiError({ kind: 'server', status: 503, message: 'private detail' });
        return decode(options, { items: [], page: 1, page_size: 100, total: 0, pages: 0, has_next: false, has_previous: false });
      }
      throw new Error(`Unexpected request ${options.path}`);
    };
    renderPage(request);
    ...
    expect(screen.getByRole('heading', { level: 2, name: 'Course outline' })).toBe(document.activeElement);
  ...
  it('renders invalid-response recovery when lesson pagination metadata drifts across pages', async () => {
    let outlineAttempts = 0;
    const firstPage = Array.from({ length: 100 }, (_, index) => ({ ...lesson(null), id: index + 1 }));
    const secondPage = Array.from({ length: 50 }, (_, index) => ({ ...lesson(null), id: index + 101 }));
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/courses/7') return decode(options, course);
      if (options.path === '/courses/7/lessons') {
        outlineAttempts += 1;
        const payload = outlineAttempts === 1
          ? { items: firstPage, page: 1, page_size: 100, total: 101, pages: 2, has_next: true, has_previous: false }
          : outlineAttempts === 2
            ? { items: secondPage, page: 2, page_size: 100, total: 150, pages: 2, has_next: false, has_previous: true }
            : { items: [], page: 1, page_size: 100, total: 0, pages: 0, has_next: false, has_previous: false };
        try {
          return decode(options, payload);
        } catch (cause) {
          throw new ApiError({ kind: 'invalid_response', status: 200, message: 'Server returned an invalid success response', cause });
        }
      }
      throw new Error(`Unexpected request ${options.path}`);
    };
    ...
    expect(await screen.findByText('The server returned an invalid response. Try again.')).toBeTruthy();
    await act(async () => { await userEvent.setup().click(screen.getByRole('button', { name: 'Try again' })); });
    expect(await screen.findByText('No lessons have been added yet.')).toBeTruthy();
  ...
  it('moves focus to recovered detail content and announces a detail retry success', async () => {
    ...
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/courses/7') {
        detailAttempts += 1;
        if (detailAttempts === 1) throw new ApiError({ kind: 'server', status: 503, message: 'private detail' });
  ...
  it('shows an authenticated Draft without preflight reads or a mutation', async () => {
    ...
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      ...
      if (options.path === '/courses/7') return decode(options, { ...course, published_at: null, price: '9.99' });
      if (options.path === '/courses/7/lessons') return decode(options, outline(null));
      throw new Error(`Unexpected request ${options.path}`);
    };
    ...
    expect((screen.getByRole('button', { name: 'Course is not published' }) as HTMLButtonElement).disabled).toBe(true);
  ...
  it('deduplicates a pending free enrollment and disables repeat submit', async () => {
    let resolveEnrollment: (() => void) | undefined;
    let mutationCount = 0;
    const pending = new Promise<void>((resolve) => { resolveEnrollment = resolve; });
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, studentProfile);
      if (options.path === '/courses/7') return decode(options, course);
      if (options.path === '/courses/7/lessons') return decode(options, outline(null));
    ...
    fireEvent.click(button);
    await waitFor(() => expect((screen.getByRole('button', { name: 'Please wait…' }) as HTMLButtonElement).disabled).toBe(true));
    expect(screen.getByRole('button', { name: 'Please wait…' }).getAttribute('aria-busy')).toBe('true');
    await waitFor(() => expect(mutationCount).toBe(1));
    resolveEnrollment?.();
    await waitFor(() => expect(screen.getByText('You are now enrolled in this course.')).toBeTruthy());
  });

  it('uses distinct paid-cart success copy without claiming course access', async () => {
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, studentProfile);
    ...
    await act(async () => { await userEvent.setup().click(addToCart); });
    expect(await screen.findByText('This course was added to your cart.')).toBeTruthy();
  ...
  it.each([
  ...
  ])('fails closed when %s returns a malformed success payload', async (mutationPath, price, actionLabel, payload) => {
    ...
    expect((screen.getByRole('button', { name: 'Action unavailable' }) as HTMLButtonElement).disabled).toBe(true);
  ...
  it('fails preflight closed and sends no mutation when enrollment pagination is invalid', async () => {
    ...
    expect((await screen.findByRole('button', { name: 'Action unavailable' }) as HTMLButtonElement).disabled).toBe(true);
    expect(await screen.findByRole('button', { name: 'Try again' })).toBeTruthy();
    expect(mutationRequests).toBe(0);
  });

  it.each([
    [new ApiError({ kind: 'unauthorized', status: 401, message: 'Could not validate credentials' }), 'Log in to enroll free', 1],
    [new ApiError({ kind: 'forbidden', status: 403, message: 'private forbidden detail' }), 'Action unavailable', 0],
    [new ApiError({ kind: 'not_found', status: 404, message: 'Course not found' }), 'Action unavailable', 1],
    [new ApiError({ kind: 'bad_request', status: 400, message: 'Course is not published' }), 'Action unavailable', 1],
    [new ApiError({ kind: 'bad_request', status: 400, message: 'Cannot enroll' }), 'Action unavailable', 0],
    [new ApiError({ kind: 'validation', status: 422, message: 'private validation detail' }), 'Action unavailable', 0],
    [new ApiError({ kind: 'conflict', status: 409, message: 'Unexpected conflict' }), 'Action unavailable', 0],
  ])('fails closed after terminal mutation outcome %# and performs the exact detail refresh', async (mutationError, expectedLabel, expectedDetailDelta) => {
    let detailRequests = 0;
    let mutationRequests = 0;
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, studentProfile);
      if (options.path === '/courses/7') { detailRequests += 1; return decode(options, course); }
      if (options.path === '/courses/7/lessons') return decode(options, outline(null));
      if (options.path === '/cart') return decode(options, emptyCart);
      if (options.path === '/enrollments/my') return decode(options, emptyEnrollments);
      if (options.path === '/enrollments') { mutationRequests += 1; throw mutationError; }
      throw new Error(`Unexpected request ${options.path}`);
    };
    renderPage(request, 'token');
    const user = userEvent.setup();
    ...
    await act(async () => { await user.click(enroll); });
    const resultingAction = await screen.findByRole(expectedLabel.startsWith('Log in') ? 'link' : 'button', { name: expectedLabel });
    if (resultingAction instanceof HTMLButtonElement) expect(resultingAction.disabled).toBe(true);
    ...
    await waitFor(() => expect(detailRequests).toBe(detailRequestsBeforeMutation + expectedDetailDelta));
    if (resultingAction instanceof HTMLButtonElement) fireEvent.click(resultingAction);
    expect(mutationRequests).toBe(1);
  });

  it.each([
    ['Already enrolled in this course', 'Already enrolled', '/enrollments/my', '/enrollments', '0.00', 'Enroll free'],
    ['Course already in cart', 'Already in cart', '/cart', '/cart/items', '19.99', 'Add to cart'],
  ])('converges known conflict %s to a terminal state and refreshes only %s', async (message, expectedLabel, refreshedPath, mutationPath, price, actionLabel) => {
    ...
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      ...
      if (options.path === '/cart') return decode(options, emptyCart);
      if (options.path === '/enrollments/my') return decode(options, emptyEnrollments);
      if (options.path === mutationPath) throw new ApiError({ kind: 'conflict', status: 409, message });
      throw new Error(`Unexpected request ${options.path}`);
    };
    renderPage(request, 'token');

    const actionButton = await screen.findByRole('button', { name: actionLabel });
    await act(async () => { await userEvent.setup().click(actionButton); });
    expect((await screen.findByRole('button', { name: expectedLabel }) as HTMLButtonElement).disabled).toBe(true);
  ...
  it('refreshes both preflight owners after an unexpected conflict while keeping the action terminal', async () => {
    const counts = new Map<string, number>();
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      counts.set(options.path, (counts.get(options.path) ?? 0) + 1);
      if (options.path === '/me') return decode(options, studentProfile);
      ...
      if (options.path === '/cart') return decode(options, emptyCart);
      if (options.path === '/enrollments/my') return decode(options, emptyEnrollments);
      if (options.path === '/enrollments') throw new ApiError({ kind: 'conflict', status: 409, message: 'Unexpected conflict' });
      throw new Error(`Unexpected request ${options.path}`);
    };
    renderPage(request, 'token');
    const user = userEvent.setup();
  ...
  it.each([
  ...
  ])('retains an explicit new-attempt action only for retryable mutation errors', async (mutationError) => {
    let mutationRequests = 0;
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, studentProfile);
      if (options.path === '/courses/7') return decode(options, course);
    ...
    const enroll = await screen.findByRole('button', { name: 'Enroll free' });
    await act(async () => { await user.click(enroll); });
    expect(await screen.findByText('The action failed. Check your connection and try again.')).toBeTruthy();
  ...
  it('does not carry a completed mutation disposition to another course', async () => {
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, studentProfile);
      if (options.path === '/courses/7') return decode(options, course);
      if (options.path === '/courses/8') return decode(options, { ...course, id: 8, title: 'Course eight' });
    ...
    const enroll = await screen.findByRole('button', { name: 'Enroll free' });
    await act(async () => { await user.click(enroll); });
    expect(await screen.findByRole('button', { name: 'Already enrolled' })).toBeTruthy();
    await act(async () => { await user.click(screen.getByRole('button', { name: 'Open course 8' })); });
  ...
  it.each([
    ['success', undefined],
    ['known conflict', new ApiError({ kind: 'conflict', status: 409, message: 'Already enrolled in this course' })],
  ])('reconciles the attempted course cache after a pending route change on %s', async (_outcome, mutationError) => {
    let resolveMutation: (() => void) | undefined;
    let mutationRequests = 0;
    let enrollmentRequests = 0;
    let enrolled = false;
    const pending = new Promise<void>((resolve) => { resolveMutation = () => { enrolled = true; resolve(); }; });
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      if (options.path === '/me') return decode(options, studentProfile);
      if (options.path === '/courses/7') return decode(options, course);
      if (options.path === '/courses/8') return decode(options, { ...course, id: 8, title: 'Course eight' });
      ...
      if (options.path === '/enrollments/my') {
        ...
        return decode(options, enrolled
          ? { ...emptyEnrollments, items: [{ id: 12, user_id: 9, course_id: 7, status: 'active', created_at: '2026-07-01T00:00:00Z', updated_at: '2026-07-01T00:00:00Z', course: { id: 7, title: course.title, description: course.description, price: course.price, currency: course.currency } }], total: 1, pages: 1 }
          : emptyEnrollments);
      }
      if (options.path === '/enrollments') {
        mutationRequests += 1;
    ...
    await act(async () => { await user.click(openCourseEight); });
    expect(await screen.findByRole('heading', { level: 1, name: 'Course eight' })).toBeTruthy();
    ...
    await act(async () => { await user.click(returnToCourseSeven); });
    expect((await screen.findByRole('button', { name: 'Already enrolled' }) as HTMLButtonElement).disabled).toBe(true);
  ...
  it('ignores a late mutation outcome after the session identity changes', async () => {
    ...
    const request: ApiClient['request'] = async <TResponse, TBody>(options: ApiRequestOptions<TBody, TResponse>) => {
      ...
      if (options.path === '/cart') return decode(options, emptyCart);
      if (options.path === '/enrollments/my') return decode(options, emptyEnrollments);
      if (options.path === '/enrollments') { await pending; return decode(options, enrollmentMutation); }
      throw new Error(`Unexpected request ${options.path}`);
    };
    ...
    const enroll = await screen.findByRole('button', { name: 'Enroll free' });
    await act(async () => { await user.click(enroll); });
    await act(async () => { await user.click(screen.getByRole('button', { name: 'Clear session' })); });
    expect(await screen.findByRole('link', { name: 'Log in to enroll free' })).toBeTruthy();
    resolveEnrollment?.();
    await waitFor(() => expect(screen.queryByText('You are now enrolled in this course.')).toBeNull());
