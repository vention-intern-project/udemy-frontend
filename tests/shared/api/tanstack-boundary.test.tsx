...
describe('TanStack server-state boundary', () => {
  it('uses one stable application client and disables both retry layers', () => {
    ...
    const view = render(<AppQueryProvider><Probe /></AppQueryProvider>);
    view.rerender(<AppQueryProvider><Probe /></AppQueryProvider>);
  ...
  it('centralizes subject-scoped private keys and credential-free mutation keys', () => {
  ...
  it('matches every selected operation to its canonical auth policy and requester', () => {
    ...
    const expected = {
      'API-002': 'required', 'API-003': 'required', 'API-004': 'required',
      'API-005': 'required', 'API-006': 'required', 'API-008': 'public',
      'API-009': 'required', 'API-010': 'optional', 'API-011': 'required',
      'API-012': 'required', 'API-013': 'required', 'API-014': 'optional',
      'API-015': 'required', 'API-016': 'required', 'API-017': 'required',
      'API-018': 'required', 'API-019': 'required', 'API-020': 'required',
  ...
  it('binds operation method, path, and policy and rejects mismatched caller options', async () => {
    ...
    expect(requests.requestPublic).toHaveBeenCalledWith(expect.objectContaining({
      method: 'GET', path: '/courses', authPolicy: 'public',
    }));
    expect(requests.requestOptional).toHaveBeenCalledWith(expect.objectContaining({
      method: 'GET', path: '/courses/course-1', authPolicy: 'optional',
    }));
    expect(requests.requestOptional).toHaveBeenCalledWith(expect.objectContaining({
      method: 'GET', path: '/lessons/lesson-1', authPolicy: 'optional',
    }));
    expect(() => requestOperation(requests.session, 'API-024', {
    ...
    expect(() => requestOperation(requests.session, 'API-024', {
      method: 'POST', path: '/login', authPolicy: 'required',
  ...
  it('passes the TanStack query AbortSignal to the existing request client', async () => {
    ...
    expect(requests.requestRequired).toHaveBeenCalledWith(expect.objectContaining({
      path: '/me', signal: controller.signal, authPolicy: 'required',
    }));
  });

  it('cancels before removing only the prior subject private cache', async () => {
    const client = createAppQueryClient();
    client.setQueryData(queryKeys.private.operation('old@example.com', 'API-026', 'me'), { old: true });
    client.setQueryData(queryKeys.private.operation('new@example.com', 'API-026', 'me'), { new: true });
    const order: string[] = [];
    ...
    await cancelAndRemovePrivateQueries(client, 'old@example.com');
