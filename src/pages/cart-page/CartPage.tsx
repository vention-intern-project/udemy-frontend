...
function CheckoutFeedbackNotice({ feedback, pending, onRecoverCheckout }: CheckoutFeedbackNoticeProps) {
  if (feedback === null) return null;
  if (feedback.kind === 'checkout_accepted') return <Notice tone="info" title="Checkout accepted"><p>Mock checkout was accepted. Payment is pending; continue in My Learning.</p><Link to="/learning">Check My Learning</Link></Notice>;
  if (feedback.kind === 'recovery_required') return <Notice tone="error" title="Checkout status needs checking"><p>We could not confirm checkout. Check the cart status for updated guidance.</p><Button variant="secondary" disabled={pending} onClick={onRecoverCheckout}>Check checkout status</Button></Notice>;
  if (feedback.kind === 'checkout_status_unknown') return <Notice tone="error" title="Checkout status remains unknown"><p>Your cart still cannot prove whether checkout partially completed. Check My Learning before taking another checkout action.</p><Link to="/learning">Check My Learning</Link></Notice>;
  if (feedback.kind === 'unauthorized') return <Notice tone="error" title="Sign in required"><p>Sign in again before continuing checkout.</p><Link to={`/login?returnTo=${encodeURIComponent('/cart')}`}>Log in</Link></Notice>;
  if (feedback.kind === 'not_authorized') return <Notice tone="error" title="Checkout unavailable"><p>This checkout is not available for the current account.</p></Notice>;
  if (feedback.kind === 'conflict') return <Notice tone="error" title="Enrollment changed"><p>Your enrollment changed. Check My Learning before taking another action.</p><Link to="/learning">Check My Learning</Link></Notice>;
  if (feedback.kind === 'cart_changed') return <Notice tone="error" title="Cart changed"><p>Your cart is no longer ready for this checkout. Refresh it before trying again.</p></Notice>;
  return <Notice tone="error" title="Checkout unavailable"><p>Checkout is currently unavailable. Try again later.</p></Notice>;
}
...
export function CartPage() {
  ...
  if (!cart.data && checkout.feedback?.kind === 'unauthorized') {
    return <section className={styles.state} aria-labelledby="cart-sign-in-heading"><h1 id="cart-sign-in-heading" ref={headingRef} tabIndex={-1}>Cart</h1><div ref={checkoutNoticeRef} tabIndex={-1}><CheckoutFeedbackNotice feedback={checkout.feedback} pending={checkout.pending} onRecoverCheckout={checkout.recoverCheckout} /></div></section>;
  }
  ...
  if (!currentCart || currentCart.items.length === 0) {
    return <section className={styles.state} aria-labelledby="cart-empty-heading"><VisuallyHidden as="p" role="status" aria-live="polite">{statusMessage}</VisuallyHidden><h1 id="cart-empty-heading" ref={headingRef} tabIndex={-1}>Your cart is empty</h1><div ref={checkoutNoticeRef} tabIndex={-1}><CheckoutFeedbackNotice feedback={checkout.feedback} pending={checkout.pending} onRecoverCheckout={checkout.recoverCheckout} /></div><p>Add a course from the catalog when you are ready to learn.</p><Link className={styles.catalogLink} to="/">Browse courses</Link></section>;
  }
  ...
  return <article className={styles.page} aria-busy={isBusy || checkout.pending}>
    <VisuallyHidden as="p" role="status" aria-live="polite">{statusMessage}</VisuallyHidden>
    <header className={styles.header}><div><p className={styles.eyebrow}>Student workspace</p><h1 ref={headingRef} tabIndex={-1}>Cart</h1><p>{currentCart.itemCount} course{currentCart.itemCount === 1 ? '' : 's'} selected</p></div><aside className={styles.summary} aria-label="Cart total"><span>Total</span><strong>{currentCart.currency} {currentCart.totalPrice}</strong><Button onClick={() => checkout.checkout(checkoutRecovery)} disabled={isBusy || checkout.pending || checkout.checkoutBlocked} state={checkout.pending ? 'loading' : 'idle'} loadingLabel="Checking out…">Mock checkout</Button><Button variant="secondary" onClick={() => setClearOpen(true)} disabled={isBusy || checkout.pending}>Clear cart</Button></aside></header>
    <div ref={checkoutNoticeRef} tabIndex={-1}><CheckoutFeedbackNotice feedback={checkout.feedback} pending={checkout.pending} onRecoverCheckout={checkout.recoverCheckout} /></div>
